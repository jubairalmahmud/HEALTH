...
import { RepresentativeTemporaryDashboard } from './components/dashboards/RepresentativeTemporaryDashboard';
...
export default function App() {
  const [user, setUser] = useState<User | null>(() => {
    try {
      const savedUser = localStorage.getItem('dmb_user');
  ...
  const [activeTab, setActiveTab] = useState<string>(() => {
    try {
      ...
      if (idFromUrl || tabParam === 'verify') return 'verify';
      if (roleParam || tabParam === 'login' || window.location.hash.includes('role=')) return 'login';
      if (tabParam) return tabParam;

      const savedTab = localStorage.getItem('dmb_active_tab');
  ...
  // Sync activeTab to localStorage & URL query parameters for reload persistence
  useEffect(() => {
    try {
      localStorage.setItem('dmb_active_tab', activeTab);
  ...
  const handleLoginSuccess = (loggedInUser: User, token: string) => {
    ...
    localStorage.setItem('dmb_user', JSON.stringify(loggedInUser));
    localStorage.setItem('dmb_token', token);
  };

  const handleLogout = () => {
    setUser(null);
    localStorage.removeItem('dmb_user');
    localStorage.removeItem('dmb_token');
    localStorage.removeItem('dmb_active_tab');
    localStorage.removeItem('dmb_admin_subtab');
    const url = new URL(window.location.href);
    url.searchParams.delete('tab');
        ...
        {activeTab === 'admin' && (
          user ? (
            user.role === 'SUPER_ADMIN' || user.role === 'ADMIN_STAFF' ? (
              ...
              user.repStatus === 'PENDING' || user.repStatus === 'REJECTED' ? (
                <RepresentativeTemporaryDashboard
                  user={user}
                  onLogout={handleLogout}
                  onApproved={() => {
                    setUser(prev => prev ? { ...prev, repStatus: 'APPROVED' } : null);
                  }}
                />
              ) : (
