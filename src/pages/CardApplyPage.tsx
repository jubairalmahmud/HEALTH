...
import { MedicalCard, PaymentSettings } from '../types';
...
export const CardApplyPage: React.FC<Props> = ({ onSuccessApply, setActiveTab }) => {
  ...
  const feeAmount = paymentSettings?.fees
    ? (formData.cardTier === 'Silver'
        ? paymentSettings.fees.silverCardFee
        : formData.cardTier === 'Gold'
        ? paymentSettings.fees.goldCardFee
        : paymentSettings.fees.platinumCardFee)
    : (formData.cardTier === 'Silver' ? 200 : formData.cardTier === 'Gold' ? 350 : 500);

  const activeMethods = paymentSettings?.methods?.filter(m => m.enabled) || [
    { id: 'bKash', name: 'bKash (বিকাশ)', accountType: 'Merchant', number: '01700-000000', instructions: '১. বিকাশ অ্যাপ বা *২৪৭# ডায়াল করে Make Payment এ যান।\n২. মার্চেন্ট নম্বর ০১৭০০-০০০০০০ এ নির্ধারিত ফি পাঠান।\n৩. ট্রানজেকশন আইডি (TrxID) ইনপুট দিন।', color: 'bg-pink-600 hover:bg-pink-500', enabled: true },
    { id: 'Nagad', name: 'Nagad (নগদ)', accountType: 'Merchant', number: '01800-000000', instructions: '১. নগদ অ্যাপ বা *১৬৭# ডায়াল করে Merchant Pay এ যান।\n২. মার্চেন্ট নম্বর ০১৮০০-০০০০০০ এ নির্ধারিত ফি পাঠান।\n৩. ট্রানজেকশন আইডি (TrxID) ইনপুট দিন।', color: 'bg-orange-600 hover:bg-orange-500', enabled: true },
    { id: 'Rocket', name: 'Rocket (রকেট)', accountType: 'Personal', number: '01900-000000-5', instructions: '১. রকেট অ্যাপ বা *৩২২# ডায়াল করে Send Money এ যান।\n২. পার্সোনাল নম্বর ০১৯০০-০০০০০০-৫ এ নির্ধারিত ফি পাঠান।\n৩. ট্রানজেকশন আইডি (TrxID) ইনপুট দিন।', color: 'bg-purple-600 hover:bg-purple-500', enabled: true },
    { id: 'Cash', name: 'নগদ / কাউন্টারে', accountType: 'Counter', number: 'ডিএমবি হেল্পডেস্ক', instructions: 'গোপালগঞ্জ ডিএমবি হেল্পডেস্কে সরাসরি ফি জমা দিন।', color: 'bg-teal-600 hover:bg-teal-500', enabled: true }
  ];

  const selectedMethodObj = activeMethods.find(m => m.id === formData.paymentMethod) || activeMethods[0];
          ...
          <div className="lg:col-span-7 bg-white rounded-3xl p-6 md:p-8 border border-slate-200 shadow-sm space-y-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="p-5 bg-gradient-to-br from-slate-900 via-slate-800 to-sky-950 text-white rounded-2xl border border-slate-700 space-y-4">
                      ...
                      <button
                        ...
                        className={`p-2.5 rounded-xl border text-xs font-bold transition text-left cursor-pointer ${
                          formData.paymentMethod === pm.id
                            ? `${pm.color || 'bg-sky-600'} text-white border-white ring-2 ring-white/50 shadow-md`
                            : 'bg-slate-800 text-slate-300 border-slate-700 hover:bg-slate-700'
                        }`}
                      >
                        <div className="text-[11px] font-bold">{pm.name}</div>
                        <div className="text-[9px] opacity-90 mt-0.5 font-mono flex items-center gap-1">
                          <span className="bg-white/20 px-1 rounded text-[8px]">{pm.accountType || 'Personal'}</span>
                          <span>{pm.number}</span>
                        </div>
                      </button>
                    ))}
                ...
                <div className="p-3 bg-slate-800/90 rounded-xl border border-slate-700 text-xs text-slate-200 space-y-2">
                  <div className="flex items-center justify-between border-b border-slate-700 pb-1.5">
                    <span className="font-bold text-amber-300 text-[11px] flex items-center gap-1">
                      📌 {selectedMethodObj.name} পেমেন্ট নির্দেশিকা ({selectedMethodObj.accountType})
                    </span>
                    <span className="bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 px-2 py-0.5 rounded-full text-[10px] font-mono font-bold">
                      ফি: ৳{feeAmount} BDT
                    </span>
                  </div>

                  {selectedMethodObj.instructions ? (
                    <div className="whitespace-pre-line text-[11px] text-slate-300 leading-relaxed font-sans bg-slate-900/60 p-2.5 rounded-lg border border-slate-700/60">
                      {selectedMethodObj.instructions}
                    </div>
                  ) : null}

                  {selectedMethodObj.bankBranch && (
                    <div className="text-[10px] font-mono text-sky-300 bg-sky-950/40 p-2 rounded border border-sky-800/50">
                      🏢 শাখা: {selectedMethodObj.bankBranch} {selectedMethodObj.routingNo ? `| Routing: ${selectedMethodObj.routingNo}` : ''}
                    </div>
                  )}

                  <div className="text-[10px] text-slate-400 bg-slate-900/40 p-2 rounded flex items-center justify-between">
                    <span>প্রাপক নম্বর / অ্যাকাউন্ট: <strong className="text-white font-mono">{selectedMethodObj.number}</strong> ({selectedMethodObj.accountType})</span>
                    <span className="text-emerald-400 font-bold">মোট ফি: ৳{feeAmount}</span>
                  </div>
                </div>

                {paymentSettings?.noticeText && (
                  <div className="p-2.5 bg-amber-950/30 border border-amber-800/50 rounded-xl text-[10px] text-amber-200/90 leading-normal">
                    ⚠️ {paymentSettings.noticeText}
                  </div>
                )}
