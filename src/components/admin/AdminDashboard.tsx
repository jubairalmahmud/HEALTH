...
export const isCardExpired = (card?: { expiryDate?: string; status?: string; validUntil?: string } | null): boolean => {
...
export const AdminDashboard: React.FC<Props> = ({
  ...
  >(() => {
    try {
      ...
      const savedSubtab = localStorage.getItem('dmb_admin_subtab');
      if (savedSubtab) return savedSubtab as any;
    } catch {}
    return 'overview';
  });
  ...
  useEffect(() => {
    try {
      localStorage.setItem('dmb_admin_subtab', activeSubTab);
  ...
  // Payment Gateway & Fees Settings State
  const [paymentSettings, setPaymentSettings] = useState<PaymentSettings>({
    methods: [
      {
        ...
        number: '01700-000000',
        instructions: '১. বিকাশ অ্যাপ বা *২৪৭# ডায়াল করে Make Payment অপশনে যান।\n২. মার্চেন্ট নম্বর ০১৭০০-০০০০০০ টাইপ করে নির্ধারিত ফি প্রদান করুন।\n৩. পেমেন্ট শেষে পাওয়া Transaction ID (TrxID) নিচে ইনপুট দিন।',
        enabled: true,
      ...
      },
      {
        ...
        accountType: 'Merchant',
        number: '01800-000000',
        instructions: '১. নগদ অ্যাপ বা *১৬৭# ডায়াল করে Merchant Pay অপশনে যান।\n২. মার্চেন্ট নম্বর ০১৮০০-০০০০০০ টাইপ করে নির্ধারিত ফি প্রদান করুন।\n৩. পেমেন্ট শেষে পাওয়া Transaction ID (TrxID) নিচে ইনপুট দিন।',
        enabled: true,
      ...
      },
      {
        ...
        number: '01900-000000-5',
        instructions: '১. রকেট অ্যাপ বা *৩২২# ডায়াল করে Send Money অপশনে যান।\n২. পার্সোনাল নম্বর ০১৯০০-০০০০০০-৫ এ নির্ধারিত ফি পাঠান।\n৩. পেমেন্ট শেষে পাওয়া Transaction ID (TrxID) নিচে ইনপুট দিন।',
        enabled: true,
        color: 'bg-purple-600 hover:bg-purple-500'
      },
      {
        id: 'Bank',
        name: 'Bank Transfer (ব্যাংক অ্যাকাউন্ট)',
        accountType: 'Bank Account',
        number: '1234567890123',
        bankBranch: 'ইসলামী ব্যাংক বাংলাদেশ লিমিটেড, গোপালগঞ্জ শাখা',
        routingNo: '125270123',
        instructions: '১. ব্যাংক অ্যাকাউন্টে বা অনলাইন ব্যাংকিং (CellFin/iSmart) এর মাধ্যমে ফি জমা দিন।\n২. ট্রানজেকশন স্লিপ/রেফারেন্স নম্বর ও অ্যাকাউন্টের বিবরণ নিচে লিখুন।',
        enabled: true,
        color: 'bg-blue-600 hover:bg-blue-500'
      },
      {
        id: 'Cash',
        name: 'নগদ / কাউন্টারে (Cash at Desk)',
        accountType: 'Counter',
        number: 'ডিএমবি হেল্পডেস্ক / পার্টনার সেন্টার',
        instructions: 'গোপালগঞ্জ ডিএমবি হেল্পডেস্কে বা নির্ধারিত অনুমোদিত পার্টনার সেন্টারে সরাসরি ফি প্রদান করুন।',
        enabled: true,
    ...
    fees: {
      ...
      goldCardFee: 350,
      platinumCardFee: 500,
      representativeFee: 500,
      quickSelectPresets: [200, 350, 500, 1000]
    },
    noticeText: 'জমাকৃত ট্রানজেকশন আইডি (TrxID) এডমিন প্যানেল হতে ভেরিফাই করা হবে। ভূয়া TrxID বা অসত্য পেমেন্ট তথ্য দেওয়া হলে আবেদনটি সরাসরি বাতিল বলে গণ্য হবে।'
  });
  const [paymentSettingsLoading, setPaymentSettingsLoading] = useState(false);
  const [paymentSettingsMsg, setPaymentSettingsMsg] = useState<{ type: 'success' | 'error'; text: string } | null>(null);
  ...
  const [bulkGenForm, setBulkGenForm] = useState({ startRange: '1001', endRange: '1050', cardTier: 'Silver', photoUrl: '' });
  ...
  const handleBulkGenPhotoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    if (file.size > 5 * 1024 * 1024) {
      alert('ছবি ফাইলের সাইজ সর্বোচ্চ ৫ মেগাবাইট হতে পারবে।');
      return;
    }
    const reader = new FileReader();
    reader.onloadend = () => {
      if (typeof reader.result === 'string') {
        setBulkGenForm(prev => ({ ...prev, photoUrl: reader.result as string }));
      }
    };
  ...
  const [distForm, setDistForm] = useState({ repId: '', repName: '', repMobile: '', district: 'গোপালগঞ্জ', upazila: 'সদর', startSerialNum: '1001', endSerialNum: '1050' });
  const [isRepDropdownOpen, setIsRepDropdownOpen] = useState(false);
  const [fieldTrackingDistrictFilter, setFieldTrackingDistrictFilter] = useState('ALL');
  ...
  const [cardDesignSettings, setCardDesignSettings] = useState<CardDesignSettings>({
    ...
    qrCodeUrl: 'https://health.nit.bd/?verify',
  ...
  // Representative Profile Update Approval Modal State
  const [repProfileApprovalModal, setRepProfileApprovalModal] = useState<RepresentativeApplication | null>(null);
  const [repProfileApprovalNotes, setRepProfileApprovalNotes] = useState('');
  const [repProfileApprovalLoading, setRepProfileApprovalLoading] = useState(false);
  ...
  const fetchAdminData = async () => {
    try {
      const [resMembers, resReps, resDists, resSms, resAudit, resSmsConfig, resBanner, resSite, resCardDesign, resTestimonials, resRoles, resContent, resCirculars, resTeam, resHero, resPayment] = await Promise.all([
        ...
        fetchJsonSafe('/api/payment-settings', undefined, null)
      ]);
      ...
      if (resPayment && resPayment.methods) setPaymentSettings(resPayment);
      fetchUserPasswordList();
    } catch (e) {
  ...
  const promptDeleteTeamMember = (id: string, name: string) => {
    setTeamMemberToDelete({ id, name });
  };

  const handleExecuteDeleteTeamMember = async () => {
    if (!teamMemberToDelete) return;
    const { id, name } = teamMemberToDelete;
    setIsDeletingTeamMember(true);

    try {
      // Optimistic update
      setTeamMembersList(prev => prev.filter(m => m.id !== id));
      setTeamToastMsg({ type: 'success', text: `"${name}" কে সফলভাবে টিম তালিকা থেকে মুছে ফেলা হয়েছে!` });
      setTeamMemberToDelete(null);
      setTimeout(() => setTeamToastMsg(null), 4000);

      const res = await fetch(`/api/team-members/${encodeURIComponent(id)}`, { method: 'DELETE' });
      if (!res.ok) {
        // Fallback refresh
        const refreshed = await fetchJsonSafe('/api/team-members', undefined, []);
        if (Array.isArray(refreshed)) {
          setTeamMembersList(refreshed);
        }
      }
    } catch (err) {
      console.error('Error deleting team member', err);
      setTeamToastMsg({ type: 'error', text: 'মুছে ফেলার সময়ে ত্রুটি ঘটেছে।' });
      setTimeout(() => setTeamToastMsg(null), 4000);
    } finally {
        ...
        const pageCanvas = await html2canvas(pageEl, {
          ...
          onclone: (clonedDoc) => {
            ...
            if (exportContainer) {
              ...
              exportContainer.style.height = '1131px';
  ...
  const handleSavePaymentSettings = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        setPaymentSettingsMsg({ type: 'success', text: data.message || 'পেমেন্ট সেটিংস ও ফি সফলভাবে আপডেট করা হয়েছে!' });
        if (data.paymentSettings) setPaymentSettings(data.paymentSettings);
      } else {
        setPaymentSettingsMsg({ type: 'error', text: data.error || 'পেমেন্ট সেটিংস আপডেট করতে সমস্যা হয়েছে।' });
      }
    } catch (err) {
      setPaymentSettingsMsg({ type: 'error', text: 'সার্ভার কানেকশন ত্রুটি!' });
    } finally {
  ...
  const handleAddPaymentMethod = () => {
    const newId = `Method-${Date.now().toString().slice(-4)}`;
    const newMethod: PaymentMethodConfig = {
      id: newId,
      name: 'নতুন পেমেন্ট মেথড (New Payment Method)',
      accountType: 'Personal',
      number: '017XXXXXXXX',
      instructions: '১. অ্যাপ খুলে নির্দেশিকা অনুযায়ী ফি পাঠান।\n২. পেমেন্ট শেষে TrxID ইনপুট দিন।',
      enabled: true,
  ...
  const handleDeletePaymentMethod = (id: string) => {
    if (!confirm('আপনি কি এই পেমেন্ট মেথডটি মুছে ফেলতে চান?')) return;
    setPaymentSettings(prev => ({
      ...prev,
  ...
  const [memberToDelete, setMemberToDelete] = useState<{ cardId: string; memberName: string } | null>(null);
  ...
  // Field Representative Direct Add State
  ...
  const [newRepForm, setNewRepForm] = useState({
    ...
    assignedArea: 'গোপালগঞ্জ সদর ও আশপাশের ইউনিয়ন',
    education: 'স্নাতক / ডিগ্রি (Graduate)',
    experience: 'মার্কেটিং ও ফিল্ড সেলস অভিজ্ঞতা',
    photoUrl: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&q=80&w=400',
    dailyTarget: 2,
    weeklyTarget: 12,
    monthlyTarget: 50,
    notes: 'অ্যাডমিন প্যানেল থেকে সরাসরি যুক্ত করা প্রতিনিধি'
  });
  const [isAddingRep, setIsAddingRep] = useState(false);
  const [newRepSuccessAlert, setNewRepSuccessAlert] = useState<{ id: string; name: string; mobile: string } | null>(null);

  // Field Reps Target and Progress Management State
  const [activeRepSearch, setActiveRepSearch] = useState('');
  const [editingTargetRepId, setEditingTargetRepId] = useState<string | null>(null);
  const [repTargetForm, setRepTargetForm] = useState<{ dailyTarget?: number; weeklyTarget?: number; monthlyTarget?: number; targetMonth?: string; targetRemarks?: string }>({});
  const [repTargetSaveLoading, setRepTargetSaveLoading] = useState(false);

  // Member Edit Helper State
  ...
  const [newPkg, setNewPkg] = useState({
    ...
    partnerLabName: 'পপুলার ডায়াগনস্টিক সেন্টার, গোপালগঞ্জ'
  });
  ...
  const handleDeleteMember = (cardId: string, name: string) => {
    setMemberToDelete({ cardId, memberName: name });
  };

  const handleExecuteDeleteMember = async () => {
    if (!memberToDelete) return;
    const { cardId } = memberToDelete;
    setIsDeletingMember(true);
    try {
      setCards(prev => prev.filter(c => c.cardId !== cardId && c.memberId !== cardId));
      if (viewMemberModal?.cardId === cardId) setViewMemberModal(null);
      if (editMemberModal?.cardId === cardId) setEditMemberModal(null);

      const res = await fetch(`/api/members/${encodeURIComponent(cardId)}`, { method: 'DELETE' });
      if (!res.ok) {
  ...
  const handleEditMemberPhotoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 8 * 1024 * 1024) {
      alert('ফাইল সাইজ খুব বড়! সর্বোচ্চ ৮ মেগাবাইট ছবি আপলোড করতে পারবেন।');
      return;
    }
    const reader = new FileReader();
    reader.onload = () => {
      ...
      if (editMemberModal) {
        setEditMemberModal({ ...editMemberModal, photoUrl: result });
      }
    };
    reader.readAsDataURL(file);
  };

  const handleNewRepPhotoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    if (file.size > 8 * 1024 * 1024) {
      alert('ফাইল সাইজ খুব বড়! সর্বোচ্চ ৮ মেগাবাইট ছবি আপলোড করতে পারবেন।');
    ...
    reader.onload = () => {
      const result = reader.result as string;
      setNewRepForm(prev => ({ ...prev, photoUrl: result }));
    };
    reader.readAsDataURL(file);
  };

  const handleDirectAddRepresentative = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newRepForm.name.trim() || !newRepForm.mobile.trim()) {
      alert('প্রতিনিধির নাম এবং মোবাইল নম্বর প্রদান করা আবশ্যক!');
      return;
    }
    setIsAddingRep(true);
    try {
      const res = await fetch('/api/representatives/direct-add', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(newRepForm)
      });
      const data = await res.json();
      if (res.ok && data.application) {
        setRepApps(prev => [data.application, ...prev]);
        setShowAddRepModal(false);
        setNewRepSuccessAlert({
        ...
        alert(data.error || 'প্রতিনিধি যুক্ত করতে সমস্যা হয়েছে।');
      }
    } catch (err) {
      console.error(err);
      alert('সার্ভার এরর');
  ...
  const handleApproveRepProfileUpdate = async (repId: string) => {
    setRepProfileApprovalLoading(true);
    try {
      const res = await fetch(`/api/representatives/applications/${repId}/approve-profile-update`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ adminNotes: repProfileApprovalNotes })
      });
      const data = await res.json();
      if (res.ok && data.application) {
        alert(data.message || 'প্রতিনিধির প্রোফাইল আপডেট সফলভাবে অনুমোদন করা হয়েছে!');
        setRepApps(prev => prev.map(r => r.id === repId ? data.application : r));
        setRepProfileApprovalModal(null);
        setRepProfileApprovalNotes('');
        fetchAdminData();
      } else {
        alert(data.error || 'অনুমোদন করতে ব্যর্থ হয়েছে।');
      }
    } catch (err) {
      alert('সার্ভার সংযোগ সমস্যা।');
    } finally {
      setRepProfileApprovalLoading(false);
    }
  };

  const handleRejectRepProfileUpdate = async (repId: string) => {
    if (!confirm('আপনি কি নিশ্চিত যে এই প্রোফাইল আপডেট আবেদনটি বাতিল করতে চান?')) return;
    setRepProfileApprovalLoading(true);
    try {
      const res = await fetch(`/api/representatives/applications/${repId}/reject-profile-update`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ adminNotes: repProfileApprovalNotes })
      });
      const data = await res.json();
      if (res.ok && data.application) {
        alert(data.message || 'প্রতিনিধির প্রোফাইল আপডেট বাতিল করা হয়েছে।');
        setRepApps(prev => prev.map(r => r.id === repId ? data.application : r));
        setRepProfileApprovalModal(null);
        setRepProfileApprovalNotes('');
        fetchAdminData();
      } else {
        alert(data.error || 'বাতিল করতে ব্যর্থ হয়েছে।');
      }
    } catch (err) {
      alert('সার্ভার সংযোগ সমস্যা।');
    } finally {
  ...
  const handleSaveQuickRepTarget = async (repId: string) => {
    setRepTargetSaveLoading(true);
    try {
      const res = await fetch(`/api/representatives/applications/${repId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          targetMonth: repTargetForm.targetMonth || '2026-08',
          targetRemarks: repTargetForm.targetRemarks || undefined,
          dailyTarget: repTargetForm.dailyTarget ? Number(repTargetForm.dailyTarget) : undefined,
          weeklyTarget: repTargetForm.weeklyTarget ? Number(repTargetForm.weeklyTarget) : undefined,
          monthlyTarget: repTargetForm.monthlyTarget ? Number(repTargetForm.monthlyTarget) : undefined
        })
      });
      const data = await res.json();
      if (res.ok && data.application) {
        setRepApps(prev => prev.map(r => r.id === repId ? { ...r, ...data.application } : r));
        setEditingTargetRepId(null);
        alert('প্রতিনিধির কাজের টার্গেট ও হিস্ট্রি সফলভাবে আপডেট করা হয়েছে!');
      } else {
        alert(data.error || 'টার্গেট আপডেট করতে ব্যর্থ হয়েছে।');
      }
    } catch (e) {
      alert('সার্ভার সংযোগ সমস্যা।');
    } finally {
      setRepTargetSaveLoading(false);
    }
  };
  ...
  const handleVerifyRepPayment = async (id: string, paymentStatus: 'PAID' | 'REJECTED' | 'UNPAID') => {
    try {
      const res = await fetch(`/api/representatives/applications/${id}/verify-payment`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ paymentStatus, adminNotes: paperCheckNotes })
      });
      const data = await res.json();
      if (res.ok) {
        alert(data.message || 'পেমেন্ট স্ট্যাটাস সফলভাবে আপডেট করা হয়েছে!');
        if (paperCheckModal && paperCheckModal.id === id) {
          setPaperCheckModal({ ...paperCheckModal, paymentStatus });
        }
        fetchAdminData();
      } else {
        alert(data.error || 'পেমেন্ট স্ট্যাটাস আপডেট ব্যর্থ হয়েছে।');
      }
    } catch (err) {
      alert('সার্ভার এরর');
    }
  ...
  const handleDistributeSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setDistForm({ repId: '', repName: '', repMobile: '', district: 'গোপালগঞ্জ', upazila: 'সদর', startSerialNum: '1001', endSerialNum: '1050' });
        fetchAdminData();
      } else {
  ...
  const handleAddPackage = (e: React.FormEvent) => {
    ...
    const created: HealthPackage = {
      ...
      popular: true,
      partnerLabName: newPkg.partnerLabName || 'পপুলার ডায়াগনস্টিক সেন্টার, গোপালগঞ্জ',
      availableLabs: [newPkg.partnerLabName || 'পপুলার ডায়াগনস্টিক সেন্টার, গোপালগঞ্জ']
    };
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col md:flex-row text-slate-800">
      ...
      <aside className={`${mobileMenuOpen ? 'block' : 'hidden'} md:block w-full md:w-64 bg-slate-900 text-slate-300 flex-shrink-0 p-5 space-y-6 z-30`}>
        ...
        <nav className="space-y-1 text-xs font-medium">
          {[
            ...
            { id: 'payment-settings', label: 'পেমেন্ট মেথড ও ফি সেটিংস (Payment Settings)', icon: <CreditCard className="w-4 h-4 text-pink-400" /> },
      ...
      <main className="flex-1 p-4 sm:p-6 lg:p-8 space-y-6 overflow-y-auto">
                          ...
                          <td className="p-3">
                            {c.status === 'PENDING' ? (
                              ...
                              <div className="space-y-1">
                                <span className="px-2 py-0.5 rounded-full text-[10px] font-black bg-rose-600 text-white inline-flex items-center gap-1 shadow-sm">
                                  <AlertTriangle className="w-3 h-3 text-amber-300" /> মেয়াদ শেষ (EXPIRED)
                                </span>
                                <span className="text-[9px] text-slate-400 font-mono block">মেয়াদ: {c.expiryDate}</span>
                              </div>
                            ) : (
                              <div className="space-y-1">
                                <button
        ...
        {activeSubTab === 'reps' && (
          <div className="space-y-8 animate-fadeIn">
            <div>
              ...
              <p className="text-xs text-slate-500">অ্যাক্টিভ ফিল্ড প্রতিনিধিদের তালিকা, কাজের অগ্রগতি পর্যবেক্ষণ, টার্গেট নির্ধারণ ও পেপারস ভেরিফিকেশন</p>
            </div>

            {/* SECTION 0: ACTIVE FIELD REPRESENTATIVES PROGRESS & EDITABLE TARGETS */}
            {(() => {
              const activeRepsList = repApps.filter(r => r.status === 'APPROVED' || r.paymentStatus === 'PAID');
              const displayReps = activeRepsList.length > 0 ? activeRepsList : repApps;
              const filteredDisplayReps = displayReps.filter(r => {
                if (!activeRepSearch.trim()) return true;
                const q = activeRepSearch.toLowerCase();
                return (
              ...
              const totalAllocatedAll = repDistributions.reduce((acc, d) => acc + (d.totalCards || 0), 0);
              const totalActivatedAll = repDistributions.reduce((acc, d) => acc + (d.usedCards || 0), 0);
              const totalMonthlyTargetAll = displayReps.reduce((acc, r) => acc + (r.monthlyTarget || 50), 0);
                ...
                <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-indigo-950 text-white rounded-3xl p-6 shadow-xl border border-slate-700 space-y-6">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <h2 className="text-lg font-black text-white flex items-center gap-2">
                            অ্যাক্টিভ ফিল্ড প্রতিনিধিদের তালিকা ও কাজের অগ্রগতি
                            <span className="text-xs font-mono font-bold bg-emerald-500 text-slate-950 px-2 py-0.5 rounded-full">
                              {displayReps.length} জন সক্রিয়
                            </span>
                          </h2>
                          <p className="text-xs text-slate-300">
                            সকল প্রতিনিধির কার্ড ইস্যু অগ্রগতি ট্র্যাক করুন এবং সরাসরি দৈনিক/সাপ্তাহিক/মাসিক টার্গেট এডিট করুন
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      <div className="relative w-full sm:w-64">
                        ...
                        <input
                          ...
                          value={activeRepSearch}
                          onChange={e => setActiveRepSearch(e.target.value)}
                          className="w-full pl-9 pr-3 py-2 rounded-xl bg-slate-800/90 border border-slate-700 text-xs text-white placeholder-slate-400 focus:outline-none focus:border-emerald-500"
                        />
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    <div className="bg-slate-800/70 p-3.5 rounded-2xl border border-slate-700">
                      <span className="text-[11px] text-slate-400 font-bold block">মোট সক্রিয় প্রতিনিধি</span>
                      <span className="text-xl font-black text-emerald-400 font-mono">{displayReps.length} জন</span>
                    </div>
                    <div className="bg-slate-800/70 p-3.5 rounded-2xl border border-slate-700">
                      <span className="text-[11px] text-slate-400 font-bold block">বরাদ্দকৃত ফিজিক্যাল কার্ড</span>
                      <span className="text-xl font-black text-sky-400 font-mono">{totalAllocatedAll} টি</span>
                    </div>
                    <div className="bg-slate-800/70 p-3.5 rounded-2xl border border-slate-700">
                      <span className="text-[11px] text-slate-400 font-bold block">ফিল্ডে নিবন্ধিত কার্ড</span>
                      <span className="text-xl font-black text-amber-400 font-mono">{totalActivatedAll} টি</span>
                    </div>
                    <div className="bg-slate-800/70 p-3.5 rounded-2xl border border-slate-700">
                      <span className="text-[11px] text-slate-400 font-bold block">সম্মিলিত মাসিক লক্ষ্য</span>
                      <span className="text-xl font-black text-purple-400 font-mono">{totalMonthlyTargetAll} টি</span>
                    </div>
                  </div>

                  {/* Representatives List / Cards with Progress & Target Editor */}
                  {filteredDisplayReps.length === 0 ? (
                    <div className="p-8 text-center bg-slate-800/40 rounded-2xl border border-dashed border-slate-700 text-slate-400 text-xs">
                      কোনো অ্যাক্টিভ প্রতিনিধি পাওয়া যায়নি।
                    </div>
                  ) : (
                    <div className="space-y-4">
                      {filteredDisplayReps.map(rep => {
                        // Calculate stats for this rep
                        const repDistributionsList = repDistributions.filter(
                          d => (d.repMobile && d.repMobile === rep.mobile) || (d.repName && d.repName.toLowerCase() === rep.name.toLowerCase())
                        );
                        const allocatedCards = repDistributionsList.reduce((acc, d) => acc + (d.totalCards || 0), 0);
                        const activatedCards = repDistributionsList.reduce((acc, d) => acc + (d.usedCards || 0), 0);

                        const monthlyTarget = rep.monthlyTarget || 50;
                        const weeklyTarget = rep.weeklyTarget || Math.round(monthlyTarget / 4);
                        const dailyTarget = rep.dailyTarget || Math.round(monthlyTarget / 24);

                        const progressPercent = Math.min(100, Math.round((activatedCards / (monthlyTarget || 1)) * 100));

                        const isEditingThisTarget = editingTargetRepId === rep.id;

                        return (
                          <div
                            key={rep.id}
                            className="bg-slate-800/90 hover:bg-slate-800 rounded-2xl p-4 sm:p-5 border border-slate-700/80 transition shadow space-y-4"
                          >
                            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                              {/* Left: Rep Info */}
                              <div className="flex items-start sm:items-center gap-3.5">
                                <img
                                  src={rep.photoUrl || 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&q=80&w=300'}
                                  alt={rep.name}
                                  className="w-12 h-12 rounded-2xl object-cover border-2 border-emerald-400 shadow-md shrink-0"
                                />
                                <div>
                                  <div className="flex items-center gap-2 flex-wrap">
                                    <h3 className="font-extrabold text-white text-sm sm:text-base">{rep.name}</h3>
                                    <span className="text-[10px] font-mono font-bold bg-slate-700 text-sky-300 px-2 py-0.5 rounded-md border border-slate-600">
                                      {rep.id}
                                    </span>
                                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
                                      rep.status === 'APPROVED' ? 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/40' : 'bg-amber-500/20 text-amber-300 border border-amber-500/40'
                                    }`}>
                                      {rep.status === 'APPROVED' ? '✓ অ্যাক্টিভ প্রতিনিধি' : 'প্রতিনিধি'}
                                    </span>
                                  </div>
                                  <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-slate-300 mt-1">
                                    <span className="font-mono text-emerald-400 flex items-center gap-1">
                                      <Phone className="w-3.5 h-3.5 text-emerald-400" /> {rep.mobile}
                                    </span>
                                    <span className="flex items-center gap-1 text-slate-300">
                                      <MapPin className="w-3.5 h-3.5 text-rose-400" /> {rep.assignedArea || rep.upazila || 'সদর'}, {rep.district || 'গোপালগঞ্জ'}
                                    </span>
                                    <span className="text-[11px] text-slate-400">
                                      পদবী: <strong className="text-slate-200">{rep.circularTitle || 'ফিল্ড অফিসার'}</strong>
                                    </span>
                                  </div>
                                </div>
                              </div>
                              ...
                              <div className="flex items-center gap-2 self-end lg:self-auto">
                                <button
                                  ...
                                  onClick={() => {
                                    if (isEditingThisTarget) {
                                      ...
                                      setEditingTargetRepId(rep.id);
                                      setRepTargetForm({
                                        targetMonth: rep.targetMonth || '2026-08',
                                        targetRemarks: rep.targetRemarks || '',
                                        dailyTarget: rep.dailyTarget || dailyTarget,
                                        weeklyTarget: rep.weeklyTarget || weeklyTarget,
                                        monthlyTarget: rep.monthlyTarget || monthlyTarget
                                      });
                                    }
                                  }}
                                  className={`px-3.5 py-1.5 rounded-xl font-bold text-xs flex items-center gap-1.5 cursor-pointer transition shadow ${
                                    isEditingThisTarget
                                ...
                                >
                                  <Edit3 className="w-3.5 h-3.5" />
                                  {isEditingThisTarget ? 'টার্গেট এডিটর বন্ধ করুন' : 'টার্গেট এডিট করুন'}
                                </button>

                                <button
                                  type="button"
                                  onClick={() => setEditRepModal(rep)}
                                  className="p-2 rounded-xl bg-slate-700/80 hover:bg-slate-700 text-slate-200 cursor-pointer transition"
                                  title="পূর্ণ প্রোফাইল ও টার্গেট হিস্ট্রি সম্পাদনা"
                                >
                            ...
                            {/* Progress & Target Overview Bar */}
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 bg-slate-900/70 p-3.5 rounded-xl border border-slate-700/60">
                              {/* Card Stats */}
                              <div className="space-y-1">
                                <span className="text-[11px] text-slate-400 font-bold block">কার্ড বিতরণ ও রেজিস্ট্রেশন</span>
                                <div className="flex items-center gap-3 font-mono text-xs">
                                  <span>বরাদ্দ: <strong className="text-sky-300 font-bold">{allocatedCards}</strong> টি</span>
                                  <span>|</span>
                                  <span>নিবন্ধিত: <strong className="text-emerald-400 font-bold">{activatedCards}</strong> টি</span>
                                </div>
                              </div>
                              ...
                              <div className="space-y-1.5 md:col-span-2">
                                <div className="flex items-center justify-between text-xs">
                                  <span className="text-slate-300 font-medium flex items-center gap-1.5">
                                    ...
                                    মাসিক লক্ষ্যমাত্রা অর্জন অগ্রগতি ({activatedCards} / {monthlyTarget} টি)
                                    {rep.targetMonth && (
                            ...
                            {/* Current Targets Badges */}
                            <div className="flex flex-wrap items-center gap-2 text-xs">
                              <span className="text-[11px] text-slate-400 font-bold">বর্তমান নির্ধারিত টার্গেট:</span>
                              <span className="px-2.5 py-1 rounded-lg bg-slate-900 border border-slate-700 text-amber-300 font-mono font-bold text-[11px]">
                                মাস: {rep.targetMonth || '2026-08'}
                              </span>
                              <span className="px-2.5 py-1 rounded-lg bg-slate-900 border border-slate-700 text-emerald-300 font-mono font-bold text-[11px]">
                                দৈনিক: {dailyTarget} টি
                              </span>
                              <span className="px-2.5 py-1 rounded-lg bg-slate-900 border border-slate-700 text-sky-300 font-mono font-bold text-[11px]">
                                সাপ্তাহিক: {weeklyTarget} টি
                              </span>
                              <span className="px-2.5 py-1 rounded-lg bg-slate-900 border border-slate-700 text-purple-300 font-mono font-bold text-[11px]">
                                মাসিক: {monthlyTarget} টি
                              </span>
                            </div>

                            {/* INLINE EDITABLE TARGET FORM */}
                            {isEditingThisTarget && (
                              <div className="bg-slate-950 p-4 rounded-2xl border-2 border-amber-500/70 space-y-3 animate-fadeIn">
                                <div className="flex items-center justify-between border-b border-slate-800 pb-2">
                                  <h4 className="font-extrabold text-amber-400 text-xs flex items-center gap-1.5">
                                    <Edit3 className="w-4 h-4" />
                                    {rep.name}-এর কাজের টার্গেট ও মাস আপডেট করুন
                                  </h4>
                                  <span className="text-[10px] text-slate-400">
                                    মাস নির্বাচন করে টার্গেট দিন এবং "সংরক্ষণ করুন" চাপুন
                                  </span>
                                </div>

                                <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 text-xs">
                                  <div>
                                    <label className="block text-slate-300 font-bold mb-1">📅 টার্গেট মাস (Month)</label>
                                    <input
                                      type="month"
                                      value={repTargetForm.targetMonth || '2026-08'}
                                      onChange={e => setRepTargetForm({ ...repTargetForm, targetMonth: e.target.value })}
                                      className="w-full p-2 rounded-xl bg-slate-900 border border-amber-500/80 text-amber-300 font-mono font-bold focus:border-amber-400 focus:outline-none"
                                    />
                                  </div>
                                  <div>
                                    <label className="block text-slate-300 font-bold mb-1">দৈনিক টার্গেট (টি/দিন)</label>
                                    <input
                                      type="number"
                                      min={1}
                                      value={repTargetForm.dailyTarget || ''}
                                      onChange={e => setRepTargetForm({ ...repTargetForm, dailyTarget: Number(e.target.value) })}
                                      className="w-full p-2 rounded-xl bg-slate-900 border border-slate-700 text-white font-mono font-bold focus:border-amber-400 focus:outline-none"
                                    />
                                  </div>
                                  <div>
                                    <label className="block text-slate-300 font-bold mb-1">সাপ্তাহিক টার্গেট (টি/সপ্তাহ)</label>
                                    <input
                                      type="number"
                                      min={1}
                                      value={repTargetForm.weeklyTarget || ''}
                                      onChange={e => setRepTargetForm({ ...repTargetForm, weeklyTarget: Number(e.target.value) })}
                                      className="w-full p-2 rounded-xl bg-slate-900 border border-slate-700 text-white font-mono font-bold focus:border-amber-400 focus:outline-none"
                                    />
                                  </div>
                                  <div>
                                    <label className="block text-slate-300 font-bold mb-1">মাসিক টার্গেট (টি/মাস) *</label>
                                    <input
                                      type="number"
                                      min={1}
                                      value={repTargetForm.monthlyTarget || ''}
                                      onChange={e => setRepTargetForm({ ...repTargetForm, monthlyTarget: Number(e.target.value) })}
                                      className="w-full p-2 rounded-xl bg-slate-900 border border-amber-400 text-white font-mono font-bold focus:border-amber-400 focus:outline-none"
                                    />
                                  </div>
                                </div>

                                <div>
                                  <label className="block text-slate-300 font-bold text-[11px] mb-1">টার্গেট সংক্রান্ত মন্তব্য / নোট (ঐচ্ছিক)</label>
                                  <input
                                    type="text"
                                    placeholder="যেমন: চলতি মাসের বিশেষ ক্যাম্পেইনে লক্ষ্যমাত্রা"
                                    value={repTargetForm.targetRemarks || ''}
                                    onChange={e => setRepTargetForm({ ...repTargetForm, targetRemarks: e.target.value })}
                                    className="w-full p-2 rounded-xl bg-slate-900 border border-slate-700 text-xs text-white placeholder-slate-500 focus:border-amber-400 focus:outline-none"
                                  />
                                </div>
                                ...
                                <div className="flex justify-end gap-2 pt-2">
                                  <button
                                    type="button"
                                    onClick={() => setEditingTargetRepId(null)}
                                    className="px-3.5 py-1.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 font-bold text-xs cursor-pointer transition"
                                  >
                                    বাতিল
                                  </button>
                                  <button
                                    type="button"
                                    disabled={repTargetSaveLoading}
                                    onClick={() => handleSaveQuickRepTarget(rep.id)}
                                    className="px-5 py-1.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold text-xs shadow cursor-pointer transition flex items-center gap-1.5"
                                  >
                                    {repTargetSaveLoading ? (
                                      <span>সংরক্ষণ হচ্ছে...</span>
                                    ) : (
                                      <>
                                        <Save className="w-3.5 h-3.5" /> টার্গেট সংরক্ষণ করুন
                                      </>
                                    )}
                                  </button>
                                </div>
            ...
            <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b pb-3">
                ...
                {/* Filters & Direct Add Button */}
                <div className="flex flex-wrap items-center gap-2 text-xs">
                  <button
                    type="button"
                    onClick={() => setShowAddRepModal(true)}
                    className="px-3.5 py-2 rounded-xl bg-sky-600 hover:bg-sky-700 text-white font-bold text-xs shadow-sm flex items-center gap-1.5 cursor-pointer transition"
                  >
                    <Plus className="w-4 h-4" /> নতুন ফিল্ড প্রতিনিধি যোগ করুন
                  </button>
                  ...
                  <table className="w-full text-left text-xs">
                    <thead className="bg-slate-900 text-slate-200 font-bold uppercase">
                      <tr>
                        ...
                        <th className="p-3">পেমেন্ট ভেরিফিকেশন</th>
                      ...
                      {repApps
                        ...
                        .map(app => (
                          <tr key={app.id} className="hover:bg-slate-50">
                            ...
                            <td className="p-3">
                              {app.paymentStatus === 'PAID' ? (
                                <div className="space-y-0.5">
                                  <span className="inline-flex items-center gap-1 bg-emerald-100 text-emerald-800 border border-emerald-300 font-extrabold text-[10px] px-2 py-0.5 rounded-full">
                                    ✓ PAID (ভেরিফাইড)
                                  </span>
                                  <p className="font-mono text-[9px] text-slate-500">{app.paymentMethod} | {app.paymentTxnId || 'Auto'}</p>
                                </div>
                              ) : app.paymentStatus === 'PENDING' ? (
                                <div className="space-y-1">
                                  <span className="inline-flex items-center gap-1 bg-amber-100 text-amber-900 border border-amber-300 font-extrabold text-[10px] px-2 py-0.5 rounded-full">
                                    ⏳ PENDING (যাচাই সাপেক্ষ)
                                  </span>
                                  <p className="font-mono text-[9px] text-slate-700 font-bold">{app.paymentMethod || 'bKash'}: {app.paymentTxnId || 'N/A'}</p>
                                  <button
                                    type="button"
                                    onClick={() => handleVerifyRepPayment(app.id, 'PAID')}
                                    className="text-[10px] bg-emerald-600 hover:bg-emerald-700 text-white font-bold px-2 py-0.5 rounded-lg shadow-sm cursor-pointer transition block"
                                  >
                                    ✓ ভেরিফাই করুন
                                  </button>
                                </div>
                            ...
                            <td className="p-3">
                              <div className="space-y-1">
                                <span className={`px-2.5 py-1 rounded-full text-[10px] font-extrabold block text-center ${
                                  app.status === 'APPROVED' 
                                ...
                                {app.pendingProfileUpdate?.status === 'PENDING' && (
                                  <span className="inline-flex items-center gap-1 bg-amber-500 text-white font-black text-[9px] px-2 py-0.5 rounded-md shadow-sm">
                                    <Clock className="w-3 h-3 animate-spin" /> আপডেট অপেক্ষমাণ
                                  </span>
                                )}
                              </div>
                            </td>
                            <td className="p-3 text-center">
                              <div className="flex items-center justify-center gap-1.5 flex-wrap">
                                {app.pendingProfileUpdate?.status === 'PENDING' && (
                                  <button
                                    onClick={() => {
                                      setRepProfileApprovalModal(app);
                                      setRepProfileApprovalNotes(app.pendingProfileUpdate?.adminNotes || '');
                                    }}
                                    className="px-2.5 py-1.5 rounded-xl bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white font-extrabold text-[10px] shadow flex items-center gap-1 cursor-pointer transition"
                                    title="প্রতিনিধির আপডেট রিভিউ করুন"
                                  >
                                    <CheckCircle2 className="w-3.5 h-3.5" /> আপডেট রিভিউ
                                  </button>
                                )}
            ...
            <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm space-y-6">
              ...
              {/* Assign Form */}
              <form onSubmit={handleDistributeSubmit} className="bg-slate-50 p-5 rounded-2xl border border-slate-200 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 text-xs">
                {/* Rep Search & Selection with Autocomplete Dropdown */}
                <div className="relative">
                  <label className="block font-bold mb-1 text-slate-800 flex items-center justify-between">
                    <span>প্রতিনিধির নাম / আইডি *</span>
                    <span className="text-[10px] text-sky-600 font-normal">অটো ড্রপডাউন</span>
                  </label>
                  <input
                    type="text"
                    required
                    ...
                    onChange={e => {
                      ...
                      setDistForm(prev => ({ ...prev, repName: val }));
                      setIsRepDropdownOpen(true);
                    }}
                    onFocus={() => setIsRepDropdownOpen(true)}
                    className="w-full p-2.5 rounded-xl border bg-white font-medium focus:ring-2 focus:ring-sky-500 outline-none"
                  />
                  ...
                  {isRepDropdownOpen && (
                    <>
                      ...
                      />
                      <div className="absolute left-0 right-0 top-full mt-1 bg-white border border-slate-200 rounded-xl shadow-xl max-h-56 overflow-y-auto z-20 divide-y divide-slate-100 animate-fadeIn">
                        {repApps
                          .filter(r => {
                            if (!distForm.repName.trim()) return true;
                            const q = distForm.repName.toLowerCase();
                            return (
                              r.name?.toLowerCase().includes(q) ||
                              r.id?.toLowerCase().includes(q) ||
                              r.mobile?.includes(q) ||
                            ...
                            <button
                              ...
                              onClick={() => {
                                setDistForm(prev => ({
                                  ...
                                  repName: r.name,
                                  repMobile: r.mobile,
                                  district: r.district || 'গোপালগঞ্জ',
                                  upazila: r.upazila || 'সদর'
                                }));
                                setIsRepDropdownOpen(false);
                              }}
                              className="w-full p-2.5 text-left hover:bg-sky-50 flex items-center justify-between gap-2 transition cursor-pointer"
                            >
                              <div className="min-w-0">
                                <p className="font-bold text-slate-900 text-xs truncate">{r.name}</p>
                                <p className="text-[10px] text-slate-500 font-mono">
                                  ID: <span className="text-sky-700 font-bold">{r.id}</span> • {r.mobile}
                                </p>
                              </div>
                              <span className="text-[10px] px-1.5 py-0.5 rounded bg-slate-100 text-slate-600 font-medium whitespace-nowrap">
                                {r.district || 'গোপালগঞ্জ'}
                              </span>
                            </button>
                          ))}
                        {repApps.filter(r => {
                ...
                <div>
                  <label className="block font-bold mb-1 text-slate-800">প্রতিনিধির মোবাইল *</label>
                  <input
                    type="text"
                    required
                    placeholder="017XXXXXXXX"
                    value={distForm.repMobile}
                    onChange={e => setDistForm(prev => ({ ...prev, repMobile: e.target.value }))}
                    className="w-full p-2.5 rounded-xl border bg-white font-mono font-bold text-slate-900"
                  />
                </div>
                <div>
                  <label className="block font-bold mb-1 text-slate-800">শুরু সিরিয়াল নম্বর *</label>
                  <input
                    type="number"
                    required
                    value={distForm.startSerialNum}
                    onChange={e => setDistForm(prev => ({ ...prev, startSerialNum: e.target.value }))}
                    className="w-full p-2.5 rounded-xl border bg-white font-mono"
                  />
                </div>
                <div>
                  <label className="block font-bold mb-1 text-slate-800">শেষ সিরিয়াল নম্বর *</label>
                  <input
                    type="number"
                    required
                    value={distForm.endSerialNum}
                    onChange={e => setDistForm(prev => ({ ...prev, endSerialNum: e.target.value }))}
                    className="w-full p-2.5 rounded-xl border bg-white font-mono"
                  />
                </div>
                <div className="flex items-end">
                  <button
                    type="submit"
                    className="w-full py-2.5 bg-sky-600 hover:bg-sky-700 text-white font-bold rounded-xl shadow cursor-pointer transition flex items-center justify-center gap-1.5"
                  >
              ...
              {/* Distributions Tracking Table with District Filter */}
              <div className="space-y-3">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-slate-50 p-3.5 rounded-xl border border-slate-200">
                  <div>
                    <h3 className="font-bold text-slate-800 text-xs">বিতরণকৃত কার্ডের ফিল্ড ট্র্যাকিং সামারি (Field Tracking Summary):</h3>
                    <p className="text-[11px] text-slate-500">জেলা ও প্রতিনিধি ভিত্তিক কার্ড বরাদ্দ এবং ফিল্ড এনরোলমেন্ট অগ্রগতি</p>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <select
                      value={fieldTrackingDistrictFilter}
                      onChange={e => setFieldTrackingDistrictFilter(e.target.value)}
                      className="p-2 rounded-xl border border-slate-300 bg-white text-xs font-bold text-slate-800 focus:ring-2 focus:ring-sky-500 outline-none"
                    >
                      ...
                      {Array.from(new Set([
                        ...
                        'বাগেরহাট',
                        'নড়াইল',
                        ...repDistributions.map(d => d.district).filter(Boolean),
                        ...repApps.map(r => r.district).filter(Boolean)
                      ])).map(dist => (
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-xs">
                    ...
                    <tbody className="divide-y divide-slate-100 font-medium">
                      {repDistributions
                        .filter(d => {
                          if (fieldTrackingDistrictFilter === 'ALL') return true;
                          const repData = repApps.find(r => r.id === d.repId || r.mobile === d.repMobile);
                          const district = d.district || repData?.district || 'গোপালগঞ্জ';
                          return district.toLowerCase() === fieldTrackingDistrictFilter.toLowerCase();
                        })
                        .map(d => {
                          const repData = repApps.find(r => r.id === d.repId || r.mobile === d.repMobile);
                          const repDistrict = d.district || repData?.district || 'গোপালগঞ্জ';
                          const repUpazila = d.upazila || repData?.upazila || 'সদর';
                          ...
                          return (
                            <tr key={d.id} className="hover:bg-slate-50">
                              <td className="p-3">
                                ...
                                <p className="text-[10px] text-slate-500 font-mono">
                                  {d.repId ? <span className="text-sky-700 font-bold mr-1">{d.repId}</span> : null}
                                  {d.repMobile}
                                </p>
                              </td>
                              <td className="p-3">
                                <span className="px-2 py-0.5 rounded-full bg-slate-100 text-slate-700 font-bold text-[11px]">
                                  {repDistrict} {repUpazila ? `(${repUpazila})` : ''}
                                </span>
                              </td>
                      ...
                      {repDistributions.filter(d => {
                        ...
                        <tr>
                          <td colSpan={8} className="p-6 text-center text-slate-400">
                            নির্বাচিত জেলায় কোনো কার্ড বিতরণের তথ্য নেই।
                          </td>
                        </tr>
                      )}
                    </tbody>
              ...
              return (
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-fadeIn">
                  ...
                  <div className="lg:col-span-7 bg-white rounded-2xl p-6 border border-slate-200 shadow-sm space-y-5">
                    ...
                    <form onSubmit={handleSaveCardDesignSettings} className="space-y-4 text-xs">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className="sm:col-span-2">
                          <div className="flex items-center justify-between mb-1">
                            <label className="font-bold text-slate-800">কার্ড স্লোগান (Card Slogan)</label>
                          ...
                          <textarea
                            ...
                            className="w-full p-3 rounded-xl border border-slate-300 bg-slate-50 font-semibold text-slate-900 text-sm focus:bg-white focus:ring-2 focus:ring-blue-500 transition-all"
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div className="sm:col-span-2">
                        <label className="block font-bold mb-1 text-slate-800">ডিসক্লেমার ওয়ার্নিং বার্তা (Back Disclaimer Note)</label>
                        <textarea
                          rows={2}
                          value={cardDesignSettings.disclaimerText || ''}
                          onChange={e => setCardDesignSettings({ ...cardDesignSettings, disclaimerText: e.target.value })}
                          className="w-full p-2.5 rounded-xl border bg-slate-50 text-xs font-medium focus:bg-white focus:ring-2 focus:ring-blue-500 transition-all"
                          placeholder="⚠️ এই কার্ডটি হস্তান্তরযোগ্য নয়। সেবার সময়ে মূল কার্ড ও ভেরিফিকেশন প্রযোজ্য।"
                        />
                        <p className="text-[11px] text-amber-600 font-medium mt-1">💡 দ্রষ্টব্য: দীর্ঘ বার্তা প্রদান করলে কার্ডের পিছনে এটি সর্বোচ্চ ২ লাইনে সুবিন্যস্ত হয়ে দেখাবে।</p>
                      </div>
                      <div className="sm:col-span-2">
                        <label className="block font-bold mb-1 text-slate-800">ফুটার অর্গানাইজেশন ব্র্যান্ডিং (Footer Branding)</label>
                        <input
                          type="text"
                          value={cardDesignSettings.footerText || ''}
                          onChange={e => setCardDesignSettings({ ...cardDesignSettings, footerText: e.target.value })}
                          className="w-full p-2.5 rounded-xl border bg-slate-50 text-xs font-medium"
                        />
                      </div>
                    </div>
              ...
              <div className="space-y-6 animate-fadeIn">
                ...
                <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm space-y-6 max-w-2xl">
                  ...
                  <form onSubmit={handleBulkGenerateSubmit} className="space-y-4 text-xs">
                    ...
                    {/* BATCH MEMBER PHOTO UPLOAD FIELD */}
                    <div>
                      <label className="block font-bold mb-1 text-slate-800 flex items-center justify-between">
                        <span>সদস্যের ছবি ফাইল আপলোড (Batch Member Photo Upload)</span>
                        <span className="text-[11px] font-normal text-slate-500">(ঐচ্ছিক - ব্যাচের সব কার্ডে এই ছবি থাকবে)</span>
                      </label>
                      
                      <div className="p-3 bg-slate-50 border-2 border-dashed border-slate-300 rounded-xl space-y-3">
                        {bulkGenForm.photoUrl ? (
                          <div className="flex items-center justify-between bg-white p-2.5 rounded-lg border border-slate-200 shadow-sm">
                            <div className="flex items-center gap-3">
                              <img
                                src={bulkGenForm.photoUrl}
                                alt="Batch Member Preview"
                                className="w-12 h-12 rounded-lg object-cover border shadow-sm"
                              />
                              <div>
                                <p className="font-bold text-slate-800 text-xs">✓ ব্যাচ মেম্বার ছবি লোড হয়েছে</p>
                                <p className="text-[10px] text-emerald-600 font-bold">জেনারেট হওয়া এই ব্যাচের সকল কার্ডে এই ছবি ব্যবহৃত হবে</p>
                              </div>
                            </div>
                            <button
                              type="button"
                              onClick={() => setBulkGenForm(prev => ({ ...prev, photoUrl: '' }))}
                              className="px-2.5 py-1 bg-rose-100 text-rose-700 hover:bg-rose-200 font-bold rounded-lg text-xs cursor-pointer transition"
                            >
                              ছবি মুছুন
                            </button>
                          </div>
                        ) : (
                          <label className="flex flex-col items-center justify-center p-4 cursor-pointer hover:bg-slate-100/80 transition rounded-lg text-center">
                            <Upload className="w-6 h-6 text-slate-500 mb-1" />
                            <span className="font-bold text-slate-800 text-xs">সদস্যের ছবি ফাইল নির্বাচন করুন বা এখানে ড্রপ করুন</span>
                            <span className="text-[10px] text-slate-500 mt-0.5">JPG, PNG, WEBP (সর্বোচ্চ 5MB)</span>
                            <input
                              type="file"
                              accept="image/*"
                              onChange={handleBulkGenPhotoUpload}
                              className="hidden"
                            />
                          </label>
                        )}
                      ...
                      <p className="text-[11px] text-slate-600 mt-1 flex items-center gap-1">
                        ...
                        <span>এখানে কোনো সদস্যের ছবি আপলোড করা হলে জেনারেট হওয়া এই ব্যাচের সকল কার্ডে ওই একই ছবি প্রিন্ট ও ডিসপ্লে হবে।</span>
                      </p>
                    </div>
                ...
                <div id="generated-bulk-cards-section" className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm space-y-4">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b pb-4">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {/* BATCH MEMBER PHOTO UPDATE BUTTON */}
                      <button
                        onClick={() => {
                          ...
                          if (targetCards.length === 0) return alert('কোনো আন-অ্যাসাইনড কার্ড পাওয়া যায়নি।');
                          ...
                          input.onchange = (e: any) => {
                            const file = e.target.files?.[0];
                            if (!file) return;
                            if (file.size > 5 * 1024 * 1024) return alert('ছবি সাইজ সর্বোচ্চ ৫ মেগাবাইট হতে পারবে।');

                            const reader = new FileReader();
                            reader.onloadend = async () => {
                              if (typeof reader.result === 'string') {
                                const photoData = reader.result;
                                const confirmUpdate = confirm(`আপনি কি নিশ্চিত যে সিলেক্টেড/সকল ${targetCards.length}টি কার্ডে এই সদস্যের ছবি সেট বা স্প্রেড করবেন?`);
                                if (!confirmUpdate) return;

                                try {
                                  const res = await fetch('/api/admin/cards/bulk-update-photo', {
                                    method: 'POST',
                                    headers: { 'Content-Type': 'application/json' },
                                    body: JSON.stringify({
                                      cardIds: targetCards.map(c => c.cardId),
                                      photoUrl: photoData
                                    })
                                  });
                                  const data = await res.json();
                                  if (res.ok) {
                                    alert(`✅ সফলভাবে ${data.updatedCount || targetCards.length}টি কার্ডে নতুন সদস্যের ছবি আপলোড ও স্প্রেড করা হয়েছে!`);
                                    fetchAdminData();
                                  } else {
                                    alert(data.error || 'ছবি স্প্রেড করতে সমস্যা হয়েছে।');
                                  }
                                } catch (err) {
                                  alert('সার্ভার এরর ঘটেছে।');
                                }
                              }
                            };
                            reader.readAsDataURL(file);
                        ...
                        className="px-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs rounded-xl shadow cursor-pointer transition flex items-center gap-1.5"
                        title="সিলেক্টেড বা সকল আন-অ্যাসাইনড কার্ডে নতুন সদস্যের ছবি সেট করুন"
                      >
                        <Upload className="w-4 h-4" />
                        <span>{selectedBulkCardIds.length > 0 ? `সিলেক্টেড (${selectedBulkCardIds.length}) ছবি স্প্রেড` : 'ব্যাচ সদস্যের ছবি স্প্রেড'}</span>
                      </button>
                      ...
                      {/* BULK CARDS DELETE BUTTON */}
                      <button
                        onClick={async () => {
                          const unassigned = cards.filter(c => (c.status || '').toUpperCase() === 'UNASSIGNED');
                          const targetCardIds = selectedBulkCardIds.length > 0
                            ? selectedBulkCardIds
                            : unassigned.map(c => c.cardId);

                          if (targetCardIds.length === 0) return alert('ডিলিট করার মতো কোনো বাল্ক কার্ড নেই।');

                          const confirmMsg = selectedBulkCardIds.length > 0
                            ? `আপনি কি নিশ্চিত যে সিলেক্টেড ${targetCardIds.length}টি বাল্ক কার্ড স্থায়ীভাবে ডিলিট করতে চান?`
                            : `আপনি কি নিশ্চিত যে সকল ${targetCardIds.length}টি আন-অ্যাসাইনড বাল্ক কার্ড স্থায়ীভাবে ডিলিট করতে চান?`;
                          ...
                          try {
                            const res = await fetch('/api/admin/cards/bulk-delete', {
                              method: 'POST',
                              headers: { 'Content-Type': 'application/json' },
                              body: JSON.stringify({ cardIds: targetCardIds })
                            });
                            const data = await res.json();
                            if (res.ok) {
                              alert(`✅ ${data.message || 'বাল্ক কার্ডসমূহ ডিলিট করা হয়েছে!'}`);
                              setSelectedBulkCardIds([]);
                              fetchAdminData();
                            } else {
                              alert(data.error || 'কার্ড ডিলিট করতে সমস্যা হয়েছে।');
                            }
                          } catch (err) {
                            alert('সার্ভার এরর ঘটেছে।');
                          }
                        }}
                        className="px-4 py-2.5 bg-rose-600 hover:bg-rose-700 text-white font-bold text-xs rounded-xl shadow cursor-pointer transition flex items-center gap-1.5"
                        title="সিলেক্টেড বা সকল আন-অ্যাসাইনড কার্ড ডিলিট করুন"
                      >
                        <Trash2 className="w-4 h-4" />
                        <span>{selectedBulkCardIds.length > 0 ? `সিলেক্টেড (${selectedBulkCardIds.length}) ডিলিট` : 'বাল্ক কার্ড ডিলিট'}</span>
                      </button>
                    </div>
                  </div>
                              ...
                              <tr key={c.cardId} className={`hover:bg-slate-50 transition ${isSelected ? 'bg-amber-50/60' : ''}`}>
                                ...
                                <td className="p-3 text-center">
                                  <div className="flex items-center justify-center gap-1.5">
                                    ...
                                    <button
                                      onClick={async () => {
                                        if (!confirm(`আপনি কি নিশ্চিত যে কার্ড ${c.cardId} স্থায়ীভাবে ডিলিট করতে চান?`)) return;
                                        try {
                                          const res = await fetch('/api/admin/cards/bulk-delete', {
                                            method: 'POST',
                                            headers: { 'Content-Type': 'application/json' },
                                            body: JSON.stringify({ cardIds: [c.cardId] })
                                          });
                                          const data = await res.json();
                                          if (res.ok) {
                                            alert(`✅ কার্ড ${c.cardId} সফলভাবে ডিলিট করা হয়েছে!`);
                                            fetchAdminData();
                                          } else {
                                            alert(data.error || 'কার্ড ডিলিট করতে সমস্যা হয়েছে।');
                                          }
                                      ...
                                      className="px-2.5 py-1 bg-rose-100 hover:bg-rose-200 text-rose-700 font-bold text-[11px] rounded-lg cursor-pointer transition inline-flex items-center gap-1"
                                      title="এই কার্ডটি ডিলিট করুন"
                                    >
                                      <Trash2 className="w-3.5 h-3.5" />
                                    </button>
                                  </div>
        ...
        {activeSubTab === 'sms' && (
          <div className="space-y-6 animate-fadeIn">
            ...
            <form onSubmit={handleSaveSmsConfig} className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm space-y-5">
              ...
              {/* Server IP Whitelist Notice Box */}
              {(smsSettings.ipWhitelistRequired || smsSettings.detectedServerIp) && (
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <h4 className="font-extrabold text-amber-950 text-xs sm:text-sm">
                          [BulkSMSBD Error 1032] সার্ভার IP হোয়াইটলিস্ট করার নির্দেশিকা (IP Whitelist Required)
                        </h4>
                        <p className="text-[11px] text-amber-900 mt-0.5">
                          BulkSMSBD এপিআই থেকে সরাসরি SMS পাঠাতে হলে আপনার ক্লাউড সার্ভারের আউটবাউন্ড IP অ্যাড্রেসটি BulkSMSBD ড্যাশবোর্ডে যোগ করতে হবে।
                        </p>
                      </div>
                    </div>
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        alert("Simulated SMS (টেস্ট মোড) নির্বাচন করা হয়েছে। নিচে 'সেটিংস সংরক্ষণ করুন' বাটনে ক্লিক করুন।");
                      }}
                      className="px-3 py-1.5 bg-amber-600 hover:bg-amber-700 text-white font-bold text-xs rounded-xl shadow cursor-pointer transition flex-shrink-0"
                    >
                      ⚠️ Simulated Mode অন করুন
                    </button>
                  </div>

                  <div className="bg-white p-3 rounded-xl border border-amber-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs">
                    <div>
                      <span className="text-slate-500 block text-[10px]">আপনার বর্তমান ক্লাউড সার্ভার আউটবাউন্ড IP Address:</span>
                      <code className="font-mono font-black text-amber-900 text-sm">{smsSettings.detectedServerIp || '34.96.48.8'}</code>
                    </div>

                    <button
                      type="button"
                      onClick={() => {
                        navigator.clipboard.writeText(smsSettings.detectedServerIp || '34.96.48.8');
                        alert(`IP Address (${smsSettings.detectedServerIp || '34.96.48.8'}) ক্লিপবোর্ডে কপি করা হয়েছে!`);
                      }}
                      className="px-3 py-1.5 bg-slate-900 hover:bg-slate-800 text-white font-bold rounded-lg text-xs flex items-center gap-1.5 cursor-pointer"
                    >
                      <Copy className="w-3.5 h-3.5" />
                      IP কপি করুন ({smsSettings.detectedServerIp || '34.96.48.8'})
                    </button>
                  </div>

                  <div className="text-[11px] text-amber-900 space-y-1 bg-amber-100/60 p-2.5 rounded-xl border border-amber-200">
                    <p className="font-bold">কীভাবে BulkSMSBD তে IP Whitelist করবেন:</p>
                    <ol className="list-decimal list-inside space-y-0.5 text-[10.5px]">
                      <li><a href="https://bulksmsbd.net" target="_blank" rel="noreferrer" className="underline font-bold text-sky-800">bulksmsbd.net</a> ড্যাশবোর্ডে লগইন করুন।</li>
                      <li><strong>Phonebook / Developer / IP Whitelist</strong> সেকশনে যান।</li>
                      <li>আইপি ফিল্ডে <code className="font-mono font-bold bg-white px-1 rounded">{smsSettings.detectedServerIp || '34.96.48.8'}</code> বসিয়ে <strong>Add / Whitelist IP</strong> দিন।</li>
                      <li>আইপি যুক্ত করার পর উপরে <strong>'গেটওয়ে কানেকশন টেস্ট করুন'</strong> বাটনে ক্লিক করে টেস্ট করে নিশ্চিত হয়ে নিন।</li>
                    </ol>
                  </div>
                </div>
              )}
              ...
              <form onSubmit={handleSaveCardDesignSettings} className="space-y-6 text-xs">
                ...
                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="sm:col-span-2">
                      <label className="block font-bold text-slate-800 mb-1">কার্ড স্লোগান / প্রধান বার্তা (Card Slogan)</label>
                      <textarea
                        rows={3}
                        required
                        value={cardDesignSettings.slogan}
                        onChange={e => setCardDesignSettings({ ...cardDesignSettings, slogan: e.target.value })}
                        className="w-full p-3 rounded-xl border border-slate-300 bg-white font-semibold text-slate-900 text-sm focus:ring-2 focus:ring-blue-500 transition-all"
                        placeholder="স্মার্ট স্বাস্থ্য সেবায় আপনার নির্ভরযোগ্য ডিজিটাল হেলথ পার্টনার"
                      />
                      <p className="text-xs text-slate-500 mt-1">কার্ডে প্রদর্শনযোগ্য মূল স্লোগান বা স্লোগানের বড় টেক্সট।</p>
                    </div>
                    ...
                    <div className="sm:col-span-2">
                      <label className="block font-bold text-slate-800 mb-1">কিউআর কোড লিংক / ভেরিফিকেশন ইউআরএল (QR Code Link)</label>
                      <input
                        type="text"
                        value={cardDesignSettings.qrCodeUrl || 'https://health.nit.bd/?verify'}
                        onChange={e => setCardDesignSettings({ ...cardDesignSettings, qrCodeUrl: e.target.value })}
                        className="w-full p-2.5 rounded-xl border border-slate-300 bg-white font-mono text-sky-800 font-bold"
                        placeholder="https://health.nit.bd/?verify"
                      />
                      <p className="text-xs text-slate-500 mt-1">কার্ডের কিউআর কোড স্ক্যান করলে ব্যবহারকারী সরাসরি এই লিংকে প্রবেশ করবে।</p>
                    </div>
                    ...
                    <div className="sm:col-span-2">
                      ...
                      <textarea
                        ...
                        value={cardDesignSettings.disclaimerText}
                        onChange={e => setCardDesignSettings({ ...cardDesignSettings, disclaimerText: e.target.value })}
                        className="w-full p-3 rounded-xl border border-slate-300 bg-white font-medium text-sm focus:ring-2 focus:ring-blue-500 transition-all"
                        placeholder="⚠️ এই কার্ডটি হস্তান্তরযোগ্য নয়। সেবার সময়ে মূল কার্ড ও ভেরিফিকেশন প্রযোজ্য।"
                      />
                      <p className="text-xs text-amber-600 font-medium mt-1">💡 দ্রষ্টব্য: দীর্ঘ সতর্কবার্তা লিখলে কার্ডের পিছনে এটি স্বয়ংক্রিয়ভাবে ২ লাইনে সুন্দরভাবে সেটিং হয়ে যাবে।</p>
                    </div>
                  </div>
                </div>
        ...
        {/* PAYMENT SETTINGS & APPLICATION FEE MANAGEMENT TAB */}
        {activeSubTab === 'payment-settings' && (
          <div className="space-y-6 animate-fadeIn">
            {/* Header */}
            <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm flex flex-wrap items-center justify-between gap-4">
              <div>
                <span className="text-[10px] font-extrabold text-pink-700 bg-pink-50 px-3 py-1 rounded-full uppercase tracking-wider flex items-center gap-1.5 w-fit">
                  <CreditCard className="w-3.5 h-3.5 text-pink-500" /> Payment Gateway & Application Fee Settings
                </span>
                <h2 className="text-xl font-bold text-slate-900 mt-2">
                  পেমেন্ট গেটওয়ে, অ্যাকাউন্ট নম্বর ও আবেদন ফি সেটিংস
                </h2>
                <p className="text-xs text-slate-600 mt-1">
                  মেডিক্যাল কার্ড আবেদন (Silver/Gold/Platinum) এবং ফিল্ড রিপ্রেজেন্টেটিভ আবেদন ফি, বিকাশ/নগদ/রকেট/ব্যাংক মার্চেন্ট নম্বর, নাম্বারের ধরন ও পেমেন্ট নির্দেশাবলী ডায়নামিক্যালি পরিচালনা করুন।
                </p>
              </div>
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="px-4 py-2 bg-pink-600 hover:bg-pink-700 text-white rounded-xl font-bold text-xs shadow flex items-center gap-2 cursor-pointer transition"
                >
                  <Plus className="w-4 h-4" /> নতুন পেমেন্ট মেথড যোগ করুন
                </button>
              </div>
            </div>
            ...
            {paymentSettingsMsg && (
              <div
                className={`p-4 rounded-2xl text-xs font-bold flex items-center justify-between ${
                  paymentSettingsMsg.type === 'success'
                    ? 'bg-emerald-50 border border-emerald-200 text-emerald-800'
                    : 'bg-rose-50 border border-rose-200 text-rose-800'
                }`}
              >
                <span>{paymentSettingsMsg.text}</span>
                <button onClick={() => setPaymentSettingsMsg(null)} className="opacity-60 hover:opacity-100">
                  <X className="w-4 h-4" />
                </button>
              </div>
            )}
            ...
            <form onSubmit={handleSavePaymentSettings} className="space-y-6">
              {/* Application Fees Configuration Box */}
              <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm space-y-4">
                <div className="flex items-center gap-2 border-b border-slate-100 pb-3">
                  <DollarSign className="w-5 h-5 text-emerald-600" />
                  <h3 className="font-bold text-slate-900 text-sm">১. কার্ড ও প্রতিনিধি আবেদন ফি (Application Fee Structure)</h3>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  <div className="p-4 bg-slate-50/80 rounded-2xl border border-slate-200 space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <input
                        type="number"
                        min="0"
                        value={paymentSettings.fees.silverCardFee}
                        onChange={e => setPaymentSettings({
                          ...paymentSettings,
                          fees: { ...paymentSettings.fees, silverCardFee: Number(e.target.value) }
                        })}
                        className="w-full pl-7 pr-3 py-2 bg-white rounded-xl border border-slate-300 font-mono text-xs font-bold text-slate-900 focus:outline-none focus:ring-2 focus:ring-sky-500"
                      />
                    </div>
                    <p className="text-[10px] text-slate-500">মেডিক্যাল সিলভার কার্ড আবেদন ফি</p>
                  </div>
                  ...
                  <div className="p-4 bg-amber-50/50 rounded-2xl border border-amber-200 space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <input
                        type="number"
                        min="0"
                        value={paymentSettings.fees.goldCardFee}
                        onChange={e => setPaymentSettings({
                          ...paymentSettings,
                          fees: { ...paymentSettings.fees, goldCardFee: Number(e.target.value) }
                        })}
                        className="w-full pl-7 pr-3 py-2 bg-white rounded-xl border border-amber-300 font-mono text-xs font-bold text-amber-900 focus:outline-none focus:ring-2 focus:ring-amber-500"
                      />
                    </div>
                    <p className="text-[10px] text-amber-700">মেডিক্যাল গোল্ড কার্ড আবেদন ফি</p>
                  </div>
                  ...
                  <div className="p-4 bg-purple-50/50 rounded-2xl border border-purple-200 space-y-2">
                    ...
                    <div className="relative">
                      <span className="absolute left-3 top-2.5 text-purple-500 font-bold text-xs">৳</span>
                      <input
                        type="number"
                        min="0"
                        value={paymentSettings.fees.platinumCardFee}
                        onChange={e => setPaymentSettings({
                          ...paymentSettings,
                          fees: { ...paymentSettings.fees, platinumCardFee: Number(e.target.value) }
                        })}
                        className="w-full pl-7 pr-3 py-2 bg-white rounded-xl border border-purple-300 font-mono text-xs font-bold text-purple-900 focus:outline-none focus:ring-2 focus:ring-purple-500"
                      />
                    </div>
                    <p className="text-[10px] text-purple-700">মেডিক্যাল প্ল্যাটিনাম কার্ড আবেদন ফি</p>
                  </div>

                  <div className="p-4 bg-emerald-50/50 rounded-2xl border border-emerald-200 space-y-2">
                    <label className="block text-xs font-bold text-emerald-900">
                      Representative (প্রতিনিধি) ফি (৳)
                    </label>
                    <div className="relative">
                      <span className="absolute left-3 top-2.5 text-emerald-500 font-bold text-xs">৳</span>
                      <input
                        type="number"
                        min="0"
                        value={paymentSettings.fees.representativeFee}
                        onChange={e => setPaymentSettings({
                          ...paymentSettings,
                          fees: { ...paymentSettings.fees, representativeFee: Number(e.target.value) }
                        })}
                        className="w-full pl-7 pr-3 py-2 bg-white rounded-xl border border-emerald-300 font-mono text-xs font-bold text-emerald-900 focus:outline-none focus:ring-2 focus:ring-emerald-500"
                      />
                    </div>
                    <p className="text-[10px] text-emerald-700">ফিল্ড রিপ্রেজেন্টেটিভ রেজিস্টার ফি</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm space-y-4">
                <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="font-bold text-slate-900 text-sm">২. পেমেন্ট গেটওয়ে, মার্চেন্ট নম্বর ও নির্দেশাবলী (Payment Gateways & Instruction)</h3>
                  </div>
                  <span className="text-xs font-bold text-slate-500 bg-slate-100 px-3 py-1 rounded-full">
                    মোট গেটওয়ে: {paymentSettings.methods.length} টি
                  </span>
                </div>

                <div className="space-y-4">
                  {paymentSettings.methods.map((method, idx) => (
                    <div
                      key={method.id}
                      className={`p-5 rounded-2xl border transition space-y-4 ${
                        method.enabled ? 'bg-slate-50/60 border-slate-200' : 'bg-slate-100/40 border-slate-200 opacity-60'
                      }`}
                    >
                        ...
                        <div className="flex items-center gap-3">
                          <label className="flex items-center gap-2 text-xs font-bold text-slate-700 cursor-pointer">
                            <input
                              ...
                              onChange={e => {
                                const updated = [...paymentSettings.methods];
                                updated[idx].enabled = e.target.checked;
                                setPaymentSettings({ ...paymentSettings, methods: updated });
                              }}
                              className="w-4 h-4 rounded text-sky-600 focus:ring-sky-500"
                            />
                            <span>{method.enabled ? 'সক্রিয় (Active)' : 'নিষ্ক্রিয় (Disabled)'}</span>
                          </label>

                          <button
                            type="button"
                            onClick={() => handleDeletePaymentMethod(method.id)}
                            className="p-1.5 bg-rose-50 hover:bg-rose-100 text-rose-600 rounded-lg text-xs font-bold transition cursor-pointer"
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        ...
                        <div>
                          <label className="block text-[11px] font-bold text-slate-700 mb-1">
                            অ্যাাকাউন্টের ধরন (Account Type) *
                          </label>
                          <select
                          ...
                          >
                            ...
                            <option value="Personal">Personal (পার্সোনাল সেেন্ড মানি)</option>
                            <option value="Agent">Agent (এজেন্ট ক্যাশ ইন)</option>
                            <option value="Bank Account">Bank Account (ব্যাংক অ্যাকাউন্ট)</option>
                            <option value="Counter">Counter (কাউন্টার / ক্যাশ ডেস্কে)</option>
                          </select>
                        </div>

                        <div>
                          <label className="block text-[11px] font-bold text-slate-700 mb-1">
                            প্রাপক নম্বর / অ্যাকাউন্ট নম্বর *
                          </label>
                          <input
                            type="text"
                            value={method.number}
                            ...
                            className="w-full px-3 py-2 bg-white rounded-xl border border-slate-300 font-mono text-xs font-bold text-slate-900"
                          />
                        </div>
                      </div>
                      ...
                      {method.accountType === 'Bank Account' && (
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
                          <div>
                            <label className="block text-[11px] font-bold text-slate-700 mb-1">
                              ব্যাংক ও শাখা বিবরণ (Branch & Bank Name)
                            </label>
                            <input
                              type="text"
                              ...
                              onChange={e => {
                                const updated = [...paymentSettings.methods];
                                updated[idx].bankBranch = e.target.value;
                                setPaymentSettings({ ...paymentSettings, methods: updated });
                              }}
                              placeholder="যেমন: ইসলামী ব্যাংক বাংলাদেশ লিমিটেড, গোপালগঞ্জ শাখা"
                          ...
                          <div>
                            ...
                            <input
                              type="text"
                              value={method.routingNo || ''}
                              onChange={e => {
                                const updated = [...paymentSettings.methods];
                                updated[idx].routingNo = e.target.value;
                                setPaymentSettings({ ...paymentSettings, methods: updated });
                              }}
                              placeholder="যেমন: 125270123"
                      ...
                      <div>
                        <label className="block text-[11px] font-bold text-slate-700 mb-1">
                          পেমেন্ট নির্দেশাবলী (User Payment Instructions)
                        </label>
                        <textarea
                          rows={3}
                          ...
                          placeholder="ব্যবহারকারীর জন্য বিস্তারিত ধাপসমূহ লিখুন..."
                          className="w-full p-3 bg-white rounded-xl border border-slate-300 text-xs font-sans leading-relaxed"
                        />
                      </div>
                    </div>
                  ))}
              ...
              <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm space-y-3">
                <div className="flex items-center gap-2 border-b border-slate-100 pb-3">
                  ...
                  <h3 className="font-bold text-slate-900 text-sm">৩. পেমেন্ট সতর্কতা ও ডিসক্লেইমার নোটিশ (Notice Text)</h3>
                </div>

                <div>
                  <label className="block text-xs font-bold text-slate-700 mb-1">
                    পেমেন্ট ফর্মের নিচে প্রদর্শিত সতর্কবার্তা:
                  </label>
                  <textarea
                    rows={2}
                    value={paymentSettings.noticeText || ''}
                    onChange={e => setPaymentSettings({ ...paymentSettings, noticeText: e.target.value })}
                    className="w-full p-3 bg-amber-50/30 rounded-xl border border-amber-200 text-xs text-amber-900 leading-relaxed font-sans"
                  />
                </div>
              </div>
              ...
              <div className="flex justify-end pt-2">
                <button
                  ...
                  className="w-full sm:w-auto px-8 py-3.5 rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold text-xs shadow-lg transition flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50"
                >
                  <CheckCircle2 className="w-4 h-4" />
                  {paymentSettingsLoading ? 'সেভ করা হচ্ছে...' : 'পেমেন্ট গেটওয়ে ও ফি তথ্য সংরক্ষণ করুন'}
                </button>
              </div>
            </form>
          </div>
                ...
                .map(member => (
                  <div key={member.id} className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm hover:shadow-md transition space-y-4 flex flex-col justify-between">
                    ...
                    <div className="pt-3 border-t border-slate-100 flex items-center justify-end gap-2">
                      ...
                      <button
                        onClick={() => promptDeleteTeamMember(member.id, member.name)}
              ...
              <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
                <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl border border-slate-200 space-y-4 animate-scaleUp">
                  ...
                  <div className="text-center space-y-2">
                    <h3 className="text-lg font-bold text-slate-900">টিম সদস্য মুছে ফেলতে চান?</h3>
                    <p className="text-xs text-slate-600 leading-relaxed">
                      আপনি কি নিশ্চিত যে <strong className="text-slate-900 font-bold">"{teamMemberToDelete.name}"</strong> কে টিম ও কর্মকর্তা তালিকা থেকে মুছে ফেলতে চান? এই অ্যাকশনটি স্থায়ী হবে।
                    </p>
                  </div>
                  ...
                  <div className="pt-3 flex items-center justify-center gap-3">
                    <button
                      ...
                      onClick={() => setTeamMemberToDelete(null)}
                    ...
                    <button
                      type="button"
                      disabled={isDeletingTeamMember}
                      onClick={handleExecuteDeleteTeamMember}
                      className="flex-1 py-2.5 rounded-xl bg-rose-600 hover:bg-rose-700 text-white font-bold text-xs shadow-md transition cursor-pointer flex items-center justify-center gap-1.5"
                    >
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-6 max-w-md w-full space-y-4 animate-fadeIn">
            ...
            <form onSubmit={handleAddPackage} className="space-y-3 text-xs">
              ...
              {/* Partner Lab Search Dropdown */}
              <div className="relative">
                <label className="block font-bold mb-1">অফার প্রদানকারী পার্টনার ল্যাব নির্বাচন করুন *</label>
                <input
                  type="text"
                  required
                  placeholder="ল্যাবের নাম বা অক্ষর টাইপ করুন..."
                  value={newPkg.partnerLabName}
                  onChange={e => {
                    const val = e.target.value;
                    setNewPkg({ ...newPkg, partnerLabName: val });
                ...
                {showLabDropdown && (
                  <div className="absolute z-30 left-0 right-0 mt-1 bg-white border border-slate-200 rounded-xl shadow-xl max-h-48 overflow-y-auto divide-y divide-slate-100">
                    {centers
                      .filter(c => !newPkg.partnerLabName || c.name.toLowerCase().includes(newPkg.partnerLabName.toLowerCase()) || c.district.toLowerCase().includes(newPkg.partnerLabName.toLowerCase()))
                      .map(c => (
                        <button
                          key={c.id}
                          type="button"
                          onClick={() => {
                            setNewPkg({ ...newPkg, partnerLabName: `${c.name} (${c.district})` });
                            setShowLabDropdown(false);
                          }}
                          className="w-full text-left p-2.5 hover:bg-sky-50 text-xs flex flex-col transition cursor-pointer"
                        >
                          <span className="font-bold text-slate-900">{c.name}</span>
                          <span className="text-[10px] text-slate-500">{c.district}, {c.upazila} • মোবাইল: {c.mobile}</span>
                        </button>
                      ))}
                    {centers.length === 0 && (
                      <div className="p-2.5 text-slate-500 text-xs">কোনো পার্টনার ল্যাব পাওয়া যায়নি</div>
                    )}
                  </div>
                )}
              </div>
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-6 max-w-md w-full space-y-4 animate-fadeIn">
            ...
            <form onSubmit={handleSaveEditPackage} className="space-y-3 text-xs">
              ...
              {/* Partner Lab Search Dropdown */}
              <div className="relative">
                <label className="block font-bold mb-1">অফার প্রদানকারী পার্টনার ল্যাব</label>
                <input
                  type="text"
                  placeholder="পার্টনার ল্যাবের নাম টাইপ করুন..."
                  value={editPackageModal.partnerLabName || ''}
                  onChange={e => {
                    const val = e.target.value;
                    setEditPackageModal({ ...editPackageModal, partnerLabName: val, availableLabs: [val] });
                    setShowLabDropdown(true);
                  }}
                  onFocus={() => setShowLabDropdown(true)}
                  className="w-full p-2.5 rounded-xl border bg-slate-50 font-medium"
                />
                {showLabDropdown && (
                  <div className="absolute z-30 left-0 right-0 mt-1 bg-white border border-slate-200 rounded-xl shadow-xl max-h-40 overflow-y-auto divide-y divide-slate-100">
                    {centers
                      .filter(c => !editPackageModal.partnerLabName || c.name.toLowerCase().includes(editPackageModal.partnerLabName.toLowerCase()))
                      .map(c => (
                        <button
                          key={c.id}
                          type="button"
                          onClick={() => {
                            setEditPackageModal({
                              ...editPackageModal,
                              partnerLabName: `${c.name} (${c.district})`,
                              availableLabs: [`${c.name} (${c.district})`]
                            });
                            setShowLabDropdown(false);
                          }}
                          className="w-full text-left p-2.5 hover:bg-sky-50 text-xs flex flex-col transition cursor-pointer"
                        >
                          <span className="font-bold text-slate-900">{c.name}</span>
                          <span className="text-[10px] text-slate-500">{c.district}, {c.upazila}</span>
                        </button>
                      ))}
                  </div>
                )}
      ...
      {viewMemberModal && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-3xl p-6 max-w-2xl w-full space-y-6 shadow-2xl animate-fadeIn my-8">
            ...
            {isCardExpired(viewMemberModal) && (
              <div className="p-3 bg-rose-600 text-white font-bold rounded-2xl flex items-center gap-2.5 shadow-md">
                <AlertTriangle className="w-5 h-5 text-amber-300 flex-shrink-0 animate-bounce" />
                <div>
                  <p className="text-xs font-black">সতর্কতা: এই কার্ডটির মেয়াদ উত্তীর্ণ হয়েছে (EXPIRED)!</p>
                  <p className="text-[11px] text-rose-100 font-normal">মেয়াদ শেষ হওয়ার তারিখ: {viewMemberModal.expiryDate}। সম্পাদনা অপশন থেকে মেয়াদ বাড়িয়ে রিনিউ করা যাবে।</p>
                </div>
              </div>
            )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
              ...
              <div className="p-3 rounded-2xl bg-slate-50 border space-y-1">
                ...
                {isCardExpired(viewMemberModal) ? (
                  <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full font-black text-xs bg-rose-600 text-white shadow-sm">
                    <AlertTriangle className="w-3 h-3 text-amber-300" /> মেয়াদ শেষ (EXPIRED)
                  </span>
                ) : (
      ...
      {editMemberModal && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-3xl p-6 max-w-3xl w-full space-y-5 shadow-2xl animate-fadeIn my-8 max-h-[92vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b pb-3 sticky top-0 bg-white z-10">
              <div className="flex items-center gap-3">
                {editMemberModal.photoUrl ? (
                  <img
                    src={editMemberModal.photoUrl}
                    alt={editMemberModal.memberName}
                    className="w-12 h-12 rounded-2xl object-cover border-2 border-amber-400 shadow-sm"
                  />
                ) : (
                  <div className="w-12 h-12 rounded-2xl bg-amber-100 text-amber-700 flex items-center justify-center font-black text-base border-2 border-amber-300">
                    {editMemberModal.memberName?.charAt(0) || 'D'}
                  </div>
                )}
                <div>
                  <h3 className="font-extrabold text-slate-900 text-base flex items-center gap-2">
                    <Edit3 className="w-5 h-5 text-amber-600" />
                    মেডিক্যাল কার্ড হোল্ডারের সকল তথ্য সম্পাদনা
                  </h3>
                  <p className="text-xs text-slate-500 font-mono">কার্ড আইডি: <strong className="text-sky-700">{editMemberModal.cardId}</strong> | মেম্বার: <strong>{editMemberModal.memberName}</strong></p>
                </div>
              </div>
              <button onClick={() => setEditMemberModal(null)} className="p-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-600 cursor-pointer transition">
                <X className="w-5 h-5" />
              </button>
            </div>

            <form onSubmit={handleSaveEditMember} className="space-y-6 text-xs">
              {/* SECTION 1: CARD & VALIDITY (মেয়াদ ও কার্ড পরিচিতি) */}
              <div className="bg-amber-50/60 p-4 rounded-2xl border border-amber-200/80 space-y-3">
                <div className="flex items-center justify-between border-b border-amber-200/70 pb-2">
                  <h4 className="font-extrabold text-amber-950 text-xs flex items-center gap-1.5">
                    ...
                    কার্ড ক্যাটাগরি, স্ট্যাটাস ও ভ্যালিডিটি / মেয়াদ (Card & Validity)
                  </h4>
                  <span className="text-[10px] font-bold text-amber-800 bg-amber-100 px-2 py-0.5 rounded-md">
                    মেয়াদ নিয়ন্ত্রণ ও এক ক্লিকে বৃদ্ধি
                  </span>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div>
                    <label className="block font-bold text-slate-700 mb-1">কার্ড আইডি / নম্বর *</label>
                    <input
                      type="text"
                      required
                      value={editMemberModal.cardId}
                      onChange={e => setEditMemberModal({ ...editMemberModal, cardId: e.target.value })}
                      className="w-full p-2.5 rounded-xl border border-slate-200 bg-white font-mono font-bold text-slate-800"
                    />
                  </div>
                  <div>
                    <label className="block font-bold text-slate-700 mb-1">কার্ড টায়ার / ক্যাটাগরি *</label>
                    <select
                      value={editMemberModal.cardTier}
                      onChange={e => {
                        const tier = e.target.value as 'Silver' | 'Gold' | 'Platinum';
                        const defLimit = tier === 'Platinum' ? 8 : tier === 'Gold' ? 6 : 4;
                        setEditMemberModal({ ...editMemberModal, cardTier: tier, memberLimit: defLimit });
                      }}
                      className="w-full p-2.5 rounded-xl border border-slate-200 bg-white font-extrabold text-amber-900"
                    >
                  ...
                  <div>
                    <label className="block font-bold text-slate-700 mb-1">সদস্য সীমা (Member Limit)</label>
                    <input
                      type="number"
                      min={1}
                      max={20}
                      value={editMemberModal.memberLimit || (editMemberModal.cardTier === 'Platinum' ? 8 : editMemberModal.cardTier === 'Gold' ? 6 : 4)}
                      onChange={e => setEditMemberModal({ ...editMemberModal, memberLimit: Number(e.target.value) })}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div>
                    <label className="block font-bold text-slate-700 mb-1">কার্ড স্ট্যাটাস (Status) *</label>
                    <select
                  ...
                  <div>
                    <label className="block font-bold text-slate-700 mb-1">ইস্যুর তারিখ (Issue Date)</label>
                    <input
                      type="date"
                      value={editMemberModal.issueDate || ''}
                      onChange={e => setEditMemberModal({ ...editMemberModal, issueDate: e.target.value })}
                      className="w-full p-2.5 rounded-xl border border-slate-200 bg-white font-medium"
                    />
                  </div>
                  <div>
                    <label className="block font-bold text-slate-700 mb-1">মেয়াদ শেষ / ভ্যালিডিটি (Expiry Date) *</label>
                    <input
                      type="date"
                      required
                      value={editMemberModal.expiryDate || ''}
                      onChange={e => setEditMemberModal({ ...editMemberModal, expiryDate: e.target.value })}
                      className="w-full p-2.5 rounded-xl border-2 border-amber-400 bg-white font-mono font-bold text-amber-900"
                    />
                  </div>
                </div>

                {/* Quick Validity Extension Buttons */}
                <div className="pt-2 border-t border-amber-200/50 flex flex-wrap items-center gap-2">
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      const cur = editMemberModal.expiryDate ? new Date(editMemberModal.expiryDate) : new Date();
                      const b = isNaN(cur.getTime()) ? new Date() : cur;
                      b.setMonth(b.getMonth() + 6);
                      setEditMemberModal({ ...editMemberModal, expiryDate: b.toISOString().split('T')[0] });
                    }}
                    className="px-2.5 py-1 rounded-lg bg-amber-100 hover:bg-amber-200 text-amber-900 font-bold text-[11px] border border-amber-300 cursor-pointer transition"
                  >
                    +৬ মাস
                  </button>
                  <button
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      setEditMemberModal({ ...editMemberModal, expiryDate: '2099-12-31' });
                    }}
              ...
              {/* SECTION 2: MEMBER PROFILE & PERSONAL DETAILS */}
              <div className="space-y-3">
                <h4 className="font-extrabold text-slate-800 text-xs flex items-center gap-1.5 border-b pb-1.5">
                  <UserIcon className="w-4 h-4 text-sky-600" />
                  সদস্যের ব্যক্তিগত ও যোগাযোগের তথ্য (Personal & Contact)
                </h4>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                  ...
                  <div>
                    <label className="block font-bold text-slate-700 mb-1">NID বা জন্ম সনদ নম্বর *</label>
                    <input
                      type="text"
                      required
                ...
                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 space-y-3">
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 border-b border-slate-200 pb-2">
                    <label className="font-extrabold text-slate-800 text-xs flex items-center gap-1.5">
                      ...
                      সদস্যের ছবি (Photo Upload / Image Link)
                    </label>
                    <span className="text-[10px] text-slate-500 font-medium">কম্পিউটার/মোবাইল থেকে সরাসরি ফাইল নির্বাচন করুন অথবা লিংক দিন</span>
                  </div>

                  <div className="grid grid-cols-1 sm:grid-cols-12 gap-4 items-center">
                    {/* Left: Preview */}
                    <div className="sm:col-span-3 flex flex-col items-center justify-center p-2 bg-white rounded-xl border border-slate-200 text-center">
                      {editMemberModal.photoUrl ? (
                        <img
                          src={editMemberModal.photoUrl}
                          alt={editMemberModal.memberName}
                        ...
                        <div className="w-20 h-20 rounded-xl bg-slate-100 border-2 border-dashed border-slate-300 text-slate-400 flex flex-col items-center justify-center text-[10px] p-1 font-bold">
                          <ImageIcon className="w-6 h-6 mb-1 text-slate-300" />
                          ছবি নেই
                        </div>
                      )}
                      <span className="text-[10px] text-slate-500 font-bold mt-1">কার্ড ছবি প্রিভিউ</span>
                    </div>
                    ...
                    <div className="sm:col-span-9 space-y-2.5">
                      ...
                      <div>
                        <label className="block text-[11px] font-bold text-slate-700 mb-1">
                          ১. ডিভাইস থেকে ছবি আপলোড করুন (File Upload):
                        </label>
                        <div className="flex items-center gap-2">
                          <label className="px-3.5 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs flex items-center gap-1.5 cursor-pointer shadow-sm transition">
                          ...
                          {editMemberModal.photoUrl && (
                            <button
                              type="button"
                              onClick={() => setEditMemberModal({ ...editMemberModal, photoUrl: '' })}
                              className="px-2.5 py-2 rounded-xl bg-rose-100 hover:bg-rose-200 text-rose-700 font-bold text-xs cursor-pointer transition"
                            >
                              রিমুভ
                            </button>
                          )}
                      ...
                      <div>
                        <label className="block text-[11px] font-bold text-slate-700 mb-1">
                          ২. অথবা ফটোর ওয়েব লিংক (Photo URL Link):
                        </label>
                        <input
                          type="text"
                          placeholder="https://images.unsplash.com/..."
                          value={editMemberModal.photoUrl || ''}
                          onChange={e => setEditMemberModal({ ...editMemberModal, photoUrl: e.target.value })}
                          className="w-full p-2.5 rounded-xl border bg-white font-mono text-xs outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500"
                        />
                      </div>
                    </div>
                  </div>
              ...
              <div className="space-y-3">
                <h4 className="font-extrabold text-slate-800 text-xs flex items-center gap-1.5 border-b pb-1.5">
                  ...
                  ঠিকানা ও ভৌগোলিক অবস্থান (Address & Location)
                </h4>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  ...
                  <div>
                    <label className="block font-bold text-slate-700 mb-1">বিস্তারিত গ্রাম/ঠিকানা</label>
                    <input
                      type="text"
              ...
              {/* SECTION 4: BENEFICIARIES / FAMILY MEMBERS LIST */}
              <div className="bg-sky-50/70 p-4 rounded-2xl border border-sky-200/80 space-y-3">
                <div className="flex items-center justify-between border-b border-sky-200 pb-2">
                  <div>
                    <h4 className="font-extrabold text-sky-950 text-xs flex items-center gap-1.5">
                      ...
                      সুবিধাভোগী পরিবারের সদস্যদের তালিকা (Beneficiaries)
                    </h4>
                    <p className="text-[11px] text-sky-700">
                      অনুমোদিত সীমা: <strong>{editMemberModal.memberLimit || (editMemberModal.cardTier === 'Platinum' ? 8 : editMemberModal.cardTier === 'Gold' ? 6 : 4)} জন</strong> | বর্তমানে যুক্ত: <strong>{(editMemberModal.beneficiaries?.length || 0)} জন</strong>
                    </p>
                  </div>
                </div>

                {/* Add new beneficiary input */}
                <div className="flex gap-2">
                  <input
                    type="text"
                    placeholder="নতুন সুবিধাভোগীর নাম ও সম্পর্ক লিখুন (যেমন: রাফসান আহমেদ - পুত্র)"
                    value={newBeneficiaryInput}
                    onChange={e => setNewBeneficiaryInput(e.target.value)}
                    onKeyDown={e => {
                      if (e.key === 'Enter') {
                        e.preventDefault();
                        if (newBeneficiaryInput.trim()) {
                          const list = editMemberModal.beneficiaries ? [...editMemberModal.beneficiaries] : [];
                          list.push(newBeneficiaryInput.trim());
                          setEditMemberModal({ ...editMemberModal, beneficiaries: list });
                          setNewBeneficiaryInput('');
                        }
                      }
                    }}
                    className="flex-1 p-2.5 rounded-xl border border-sky-200 bg-white text-xs font-medium"
                  />
                  <button
                    type="button"
                    onClick={() => {
                    ...
                    className="px-4 py-2.5 bg-sky-600 hover:bg-sky-700 text-white font-bold rounded-xl shadow cursor-pointer transition flex items-center gap-1"
                  >
                ...
                {(!editMemberModal.beneficiaries || editMemberModal.beneficiaries.length === 0) ? (
                  <p className="text-[11px] text-slate-500 bg-white/70 p-3 rounded-xl border border-sky-100 text-center italic">
                    কোনো সুবিধাভোগী সদস্য যুক্ত নেই। উপরে নাম ও সম্পর্ক লিখে "যুক্ত করুন" বাটনে চাপ দিন।
                  </p>
                ) : (
                  <div className="space-y-2 max-h-48 overflow-y-auto">
                    {editMemberModal.beneficiaries.map((beneficiary, idx) => (
                      <div key={idx} className="flex items-center gap-2 bg-white p-2 rounded-xl border border-sky-100 shadow-sm">
                        <span className="w-6 h-6 rounded-lg bg-sky-100 text-sky-800 flex items-center justify-center font-bold text-[10px]">
                          {idx + 1}
                        </span>
                        <input
                          type="text"
                          value={beneficiary}
                          onChange={e => {
                            const updated = [...(editMemberModal.beneficiaries || [])];
                            updated[idx] = e.target.value;
                            setEditMemberModal({ ...editMemberModal, beneficiaries: updated });
                          }}
                          className="flex-1 p-1.5 rounded-lg border border-slate-200 bg-slate-50 font-medium text-xs text-slate-800"
                        />
                        <button
                          type="button"
                          onClick={() => {
              ...
              {/* SECTION 5: PAYMENT & FEES (পেমেন্ট তথ্য) */}
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 space-y-3">
                <h4 className="font-extrabold text-slate-800 text-xs flex items-center gap-1.5 border-b pb-1.5">
                  <DollarSign className="w-4 h-4 text-emerald-600" />
                  আবেদন ফি ও পেমেন্ট ট্রানজেকশন (Payment & Fee Details)
                </h4>

                <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
                  <div>
                    <label className="block font-bold text-slate-700 mb-1">ফি পরিমাণ (৳)</label>
                    <input
                      type="number"
                      value={editMemberModal.feeAmount || 0}
                      onChange={e => setEditMemberModal({ ...editMemberModal, feeAmount: Number(e.target.value) })}
                      className="w-full p-2.5 rounded-xl border bg-white font-mono font-bold text-slate-900"
                    />
                  </div>
                  <div>
                    ...
                    <select
                      value={editMemberModal.paymentMethod || 'Cash'}
                      onChange={e => setEditMemberModal({ ...editMemberModal, paymentMethod: e.target.value })}
                      className="w-full p-2.5 rounded-xl border bg-white font-bold"
                    >
                      ...
                      <option value="Cash">Cash at Counter (নগদ)</option>
                    </select>
                  </div>
                  <div>
                    <label className="block font-bold text-slate-700 mb-1">পেমেন্ট স্ট্যাটাস</label>
                    <select
                      value={editMemberModal.paymentStatus || 'PAID'}
                      onChange={e => setEditMemberModal({ ...editMemberModal, paymentStatus: e.target.value })}
                      className="w-full p-2.5 rounded-xl border bg-white font-bold text-emerald-800"
                    >
                  ...
                  <div>
                    <label className="block font-bold text-slate-700 mb-1">TrxID / রেফারেন্স</label>
                    <input
                      type="text"
                      placeholder="TRX..."
                      value={editMemberModal.trxId || ''}
                      onChange={e => setEditMemberModal({ ...editMemberModal, trxId: e.target.value })}
                      className="w-full p-2.5 rounded-xl border bg-white font-mono text-xs"
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between pt-4 border-t sticky bottom-0 bg-white z-10">
                <button
                  type="button"
                  onClick={() => setEditMemberModal(null)}
                  className="px-5 py-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold cursor-pointer transition"
                >
                  বাতিল
                </button>
                <button
                  type="submit"
                  className="px-7 py-2.5 rounded-xl bg-amber-600 hover:bg-amber-700 text-white font-extrabold shadow-lg cursor-pointer transition flex items-center gap-2"
                >
                  <Save className="w-4 h-4" /> সকল তথ্য সংরক্ষণ ও আপডেট করুন
                </button>
              </div>
            </form>
          </div>
          ...
          <div className="bg-white rounded-3xl p-6 max-w-2xl w-full space-y-4 shadow-2xl animate-fadeIn my-8">
            ...
            <form onSubmit={handleSaveEditRep} className="space-y-3 text-xs">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                {/* Target Settings with Month/Year Selection & Monthly History */}
                <div className="sm:col-span-2 bg-gradient-to-br from-amber-50/90 to-orange-50/70 p-4 rounded-2xl border border-amber-300 shadow-sm space-y-4">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <label className="block font-black text-amber-950 text-xs">
                          কাজের লক্ষ্যমাত্রা ও টার্গেট নির্ধারণ (Target & Performance Settings)
                        </label>
                        <p className="text-[10px] text-amber-800">
                          নির্দিষ্ট মাস ও বছরের জন্য টার্গেট সেট করুন এবং পূর্ববর্তী কাজের হিস্ট্রি পরিচালনা করুন
                        </p>
                      </div>
                    </div>
                    <span className="text-[10px] text-amber-900 bg-amber-200/80 px-2.5 py-0.5 rounded-full font-extrabold self-start sm:self-auto border border-amber-300">
                      মাসভিত্তিক ট্র্যাকিং
                    </span>
                  </div>

                  {/* Month & Year Selection */}
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 bg-white/80 p-3 rounded-xl border border-amber-200">
                    <div>
                      <label className="block font-extrabold text-slate-800 text-[11px] mb-1">
                        📅 টার্গেটের মাস ও বছর নির্বাচন (Month / Year) *
                      </label>
                      <input
                        type="month"
                        value={editRepModal.targetMonth || '2026-08'}
                        onChange={e => {
                          const newMonth = e.target.value;
                          const existingHistory = editRepModal.targetHistory?.find(h => h.month === newMonth);
                          setEditRepModal({
                            ...editRepModal,
                            targetMonth: newMonth,
                            monthlyTarget: existingHistory ? existingHistory.target : editRepModal.monthlyTarget,
                            dailyTarget: existingHistory?.dailyTarget ?? editRepModal.dailyTarget,
                            weeklyTarget: existingHistory?.weeklyTarget ?? editRepModal.weeklyTarget,
                            targetRemarks: existingHistory?.remarks ?? editRepModal.targetRemarks
                          });
                        }}
                        className="w-full p-2 rounded-xl border-2 border-amber-400 bg-amber-50/50 font-mono font-bold text-slate-900 text-xs focus:bg-white focus:outline-none"
                      />
                    </div>

                    <div>
                      <label className="block font-bold text-slate-700 text-[11px] mb-1">
                        দ্রুত মাস নির্বাচন (Quick Select)
                      </label>
                      <div className="flex flex-wrap gap-1.5 pt-0.5">
                        {[
                          { key: '2026-08', label: 'চলতি মাস (আগস্ট ২০২৬)' },
                          { key: '2026-09', label: 'পরবর্তী মাস (সেপ্টেম্বর ২০২৬)' },
                          { key: '2026-07', label: 'পূর্ববর্তী মাস (জুলাই ২০২৬)' }
                        ].map(m => (
                          <button
                            key={m.key}
                            type="button"
                            onClick={() => {
                              const existingHistory = editRepModal.targetHistory?.find(h => h.month === m.key);
                              setEditRepModal({
                                ...editRepModal,
                                targetMonth: m.key,
                                monthlyTarget: existingHistory ? existingHistory.target : editRepModal.monthlyTarget,
                            ...
                            className={`px-2.5 py-1 text-[10px] font-bold rounded-lg transition cursor-pointer ${
                              (editRepModal.targetMonth || '2026-08') === m.key
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className="w-full p-2.5 rounded-xl border-2 border-amber-500 bg-white font-mono font-black text-slate-900 text-xs"
                      />
                    </div>
                  </div>

                  {/* Target Remarks / Guidelines */}
                  <div>
                    <label className="block font-bold text-slate-700 text-[11px] mb-1">
                      টার্গেট মন্তব্য / লক্ষ্যমাত্রার নির্দেশনা (Target Remarks / Notes)
                    </label>
                    <input
                      type="text"
                      placeholder="যেমন: চলতি মাসের ক্যাম্পেইনে সর্বোচ্চ অর্জনের লক্ষ্যমাত্রা"
                      value={editRepModal.targetRemarks ?? ''}
                      onChange={e => setEditRepModal({
                        ...editRepModal,
                        targetRemarks: e.target.value
                      })}
                      className="w-full p-2 rounded-xl border border-amber-300 bg-white text-xs text-slate-800"
                    />
                  </div>

                  {/* Target & Achievement History Table */}
                  <div className="bg-white rounded-xl p-3 border border-amber-200/90 space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="font-extrabold text-xs text-slate-900 flex items-center gap-1.5">
                        <TrendingUp className="w-3.5 h-3.5 text-emerald-600" />
                        প্রতিনিধির বর্তমান ও পূর্ববর্তী মাসের টার্গেট/অর্জিত কাজের হিস্ট্রি
                      </span>
                      <span className="text-[10px] text-slate-500">
                        মোট রেকর্ড: {editRepModal.targetHistory?.length || 0} টি মাস
                      </span>
                    </div>
                    ...
                    <div className="overflow-x-auto max-h-52 overflow-y-auto">
                      <table className="w-full text-left text-[11px]">
                        <thead className="bg-slate-100 text-slate-700 font-bold sticky top-0 uppercase">
                          <tr>
                            <th className="p-2">মাস ও বছর</th>
                            <th className="p-2 text-center">নির্ধারিত টার্গেট</th>
                            <th className="p-2 text-center">অর্জিত কার্ড</th>
                            <th className="p-2 text-center">অগ্রগতি %</th>
                            <th className="p-2 text-center">স্ট্যাটাস</th>
                            <th className="p-2">মন্তব্য / বিবরণ</th>
                            <th className="p-2 text-center">একশন</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100">
                          {(!editRepModal.targetHistory || editRepModal.targetHistory.length === 0) ? (
                            <tr>
                              <td colSpan={7} className="p-4 text-center text-slate-400 text-[10px]">
                                পূর্ববর্তী কোনো টার্গেট হিস্ট্রি রেকর্ড পাওয়া যায়নি। উপরের ফর্মে টার্গেট দিয়ে সেভ করুন।
                              </td>
                            </tr>
                          ) : (
                            editRepModal.targetHistory.map((item, idx) => {
                              const rate = item.target > 0 ? Math.round((item.achieved / item.target) * 100) : 0;
                              const isCurrentSelected = (editRepModal.targetMonth || '2026-08') === item.month;
                              return (
                                <tr key={idx} className={`hover:bg-amber-50/50 ${isCurrentSelected ? 'bg-amber-50/80 font-semibold' : ''}`}>
                                  <td className="p-2 font-mono font-bold text-slate-900">
                                    <span className="inline-block px-1.5 py-0.5 rounded bg-slate-100 text-slate-800 text-[10px] mr-1">
                                      {item.month}
                                    </span>
                                    {item.month === '2026-08' ? 'চলতি মাস (আগস্ট)' : item.month === '2026-07' ? 'জুলাই ২০২৬' : item.month === '2026-06' ? 'জুন ২০২৬' : item.month === '2026-05' ? 'মে ২০২৬' : ''}
                                  </td>
                                  ...
                                  <td className="p-2 text-center">
                                    <div className="flex items-center justify-center gap-1">
                                      <span className="font-mono text-[10px] font-bold text-slate-700">{rate}%</span>
                                      <div className="w-12 bg-slate-200 h-1.5 rounded-full overflow-hidden">
                                        <div
                                          className={`h-full rounded-full ${rate >= 100 ? 'bg-emerald-500' : rate >= 50 ? 'bg-sky-500' : 'bg-rose-500'}`}
                                  ...
                                  <td className="p-2 text-center">
                                    <span className={`px-2 py-0.5 rounded-full text-[9px] font-extrabold ${
                                      ...
                                      item.status === 'IN_PROGRESS' ? 'bg-sky-100 text-sky-800' : 'bg-rose-100 text-rose-800'
                                    }`}>
                                      {item.status === 'COMPLETED' ? '✓ অর্জিত' : item.status === 'IN_PROGRESS' ? '⏳ চলমান' : '⚠️ অপূর্ণ'}
                                    </span>
                                  </td>
                                  <td className="p-2 text-slate-600 text-[10px] truncate max-w-[150px]" title={item.remarks}>
                                    {item.remarks || '-'}
                                  </td>
                                  <td className="p-2 text-center">
                                    <button
                                      ...
                                      onClick={() => {
                                        setEditRepModal({
                                          ...
                                          dailyTarget: item.dailyTarget ?? editRepModal.dailyTarget,
                                          weeklyTarget: item.weeklyTarget ?? editRepModal.weeklyTarget,
                                          targetRemarks: item.remarks ?? editRepModal.targetRemarks
                                        });
                                      }}
                                      className="px-2 py-0.5 rounded bg-amber-100 hover:bg-amber-200 text-amber-900 font-bold text-[10px] cursor-pointer"
                                      title="এই মাসের তথ্য ফর্মে লোড করুন"
                                    >
                                      লোড
                                    </button>
                                  </td>
      ...
      {/* REPRESENTATIVE PROFILE UPDATE APPROVAL MODAL */}
      {repProfileApprovalModal && (
        <div className="fixed inset-0 z-50 bg-slate-900/70 backdrop-blur-sm flex items-center justify-center p-4 animate-fadeIn overflow-y-auto">
          <div className="bg-white rounded-3xl max-w-2xl w-full shadow-2xl border border-slate-200 overflow-hidden my-8">
            <div className="bg-gradient-to-r from-amber-600 to-orange-600 text-white p-5 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-2xl bg-white/20 backdrop-blur-sm flex items-center justify-center font-bold text-white shadow-inner">
                  <Clock className="w-5 h-5" />
                </div>
                <div>
                  <h3 className="font-extrabold text-base leading-snug">
                    প্রতিনিধির প্রোফাইল তথ্য আপডেট অনুমোদন
                  </h3>
                  <p className="text-xs text-amber-100 font-medium">
                    প্রতিনিধি আইডি: <span className="font-mono font-bold text-white">{repProfileApprovalModal.id}</span>
                  </p>
                </div>
              </div>
              <button
                type="button"
                onClick={() => setRepProfileApprovalModal(null)}
            ...
            <div className="p-6 space-y-6 max-h-[75vh] overflow-y-auto">
              <div className="p-4 bg-amber-50 rounded-2xl border border-amber-200 text-xs text-amber-900 leading-relaxed">
                <p className="font-bold flex items-center gap-1.5 mb-1 text-amber-950">
                  ...
                  প্রতিনিধি কর্তৃক অনুরোধকৃত পরিবর্তনের বিবরণ
                </p>
                <p>
                  প্রতিনিধি তাঁর পোর্টাল থেকে প্রোফাইল তথ্য সংশোধনের আবেদন করেছেন। নিচে বর্তমান তথ্য এবং অনুরোধকৃত নতুন তথ্যের তুলনা দেখে নিন:
                </p>
                {repProfileApprovalModal.pendingProfileUpdate?.requestedAt && (
                  <p className="text-[10px] text-amber-700 mt-1 font-mono">
                    আবেদনের তারিখ ও সময়: {new Date(repProfileApprovalModal.pendingProfileUpdate.requestedAt).toLocaleString('bn-BD')}
                  </p>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                {/* Current Data */}
                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 space-y-3">
                  <h4 className="font-black text-slate-700 text-xs uppercase tracking-wider flex items-center gap-1.5 border-b pb-2">
                    <UserIcon className="w-4 h-4 text-slate-500" /> বর্তমান তথ্য (Current)
                  </h4>
                  <div className="space-y-2.5">
                    <div>
                      <span className="text-[10px] text-slate-500 font-bold block">প্রোফাইল ছবি:</span>
                      <img
                        src={repProfileApprovalModal.photoUrl || 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&q=80&w=300'}
                        alt="Current"
                        className="w-16 h-16 rounded-xl object-cover border border-slate-300 shadow-sm mt-1"
                      />
                    </div>
                    <div>
                      <span className="text-[10px] text-slate-500 font-bold block">প্রতিনিধি আইডি:</span>
                      <p className="font-mono font-bold text-slate-800">{repProfileApprovalModal.id}</p>
                    </div>
                    <div>
                      <span className="text-[10px] text-slate-500 font-bold block">নাম:</span>
                      <p className="font-bold text-slate-800">{repProfileApprovalModal.name}</p>
                    </div>
                    <div>
                      <span className="text-[10px] text-slate-500 font-bold block">মোবাইল নম্বর:</span>
                      <p className="font-mono font-bold text-slate-800">{repProfileApprovalModal.mobile}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-amber-50/70 p-4 rounded-2xl border-2 border-amber-300 space-y-3">
                  <h4 className="font-black text-amber-900 text-xs uppercase tracking-wider flex items-center gap-1.5 border-b border-amber-200 pb-2">
                    <CheckCircle2 className="w-4 h-4 text-amber-600" /> অনুরোধকৃত নতুন তথ্য (Requested)
                  </h4>
                  <div className="space-y-2.5">
                    <div>
                      <span className="text-[10px] text-amber-800 font-bold block">নতুন ছবি:</span>
                      {repProfileApprovalModal.pendingProfileUpdate?.pendingData?.photoUrl ? (
                        <div className="flex items-center gap-2 mt-1">
                          <img
                            src={repProfileApprovalModal.pendingProfileUpdate.pendingData.photoUrl}
                            alt="Requested New"
                        ...
                        <p className="text-slate-400 italic">অপরিবর্তিত</p>
                      )}
                    </div>
                    <div>
                      <span className="text-[10px] text-amber-800 font-bold block">প্রতিনিধি আইডি:</span>
                      <p className={`font-mono font-bold ${repProfileApprovalModal.pendingProfileUpdate?.pendingData?.id && repProfileApprovalModal.pendingProfileUpdate.pendingData.id !== repProfileApprovalModal.id ? 'text-amber-800 bg-amber-100 px-1.5 py-0.5 rounded inline-block' : 'text-slate-600'}`}>
                        {repProfileApprovalModal.pendingProfileUpdate?.pendingData?.id || repProfileApprovalModal.id}
                      </p>
                    </div>
                    <div>
                      <span className="text-[10px] text-amber-800 font-bold block">নতুন নাম:</span>
                      <p className={`font-bold ${repProfileApprovalModal.pendingProfileUpdate?.pendingData?.name && repProfileApprovalModal.pendingProfileUpdate.pendingData.name !== repProfileApprovalModal.name ? 'text-amber-900 bg-amber-100 px-1.5 py-0.5 rounded inline-block' : 'text-slate-600'}`}>
                        {repProfileApprovalModal.pendingProfileUpdate?.pendingData?.name || repProfileApprovalModal.name}
                      </p>
                    </div>
                    <div>
                      <span className="text-[10px] text-amber-800 font-bold block">নতুন মোবাইল নম্বর:</span>
                      <p className={`font-mono font-bold ${repProfileApprovalModal.pendingProfileUpdate?.pendingData?.mobile && repProfileApprovalModal.pendingProfileUpdate.pendingData.mobile !== repProfileApprovalModal.mobile ? 'text-amber-900 bg-amber-100 px-1.5 py-0.5 rounded inline-block' : 'text-slate-600'}`}>
                        {repProfileApprovalModal.pendingProfileUpdate?.pendingData?.mobile || repProfileApprovalModal.mobile}
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="space-y-1.5">
                <label className="block font-bold text-slate-800 text-xs">
                  অ্যাডমিন মন্তব্য / নোট (ঐচ্ছিক):
                </label>
                <input
                  type="text"
                  placeholder="যেমন: কাগজপত্র যাচাই সাপেক্ষে অনুমোদন করা হলো"
                  value={repProfileApprovalNotes}
                  onChange={e => setRepProfileApprovalNotes(e.target.value)}
                  className="w-full p-2.5 rounded-xl border border-slate-300 text-xs bg-slate-50 focus:bg-white focus:border-amber-500 outline-none"
                />
              </div>
              ...
              <div className="flex items-center justify-end gap-3 pt-3 border-t border-slate-200">
                <button
                  type="button"
                  disabled={repProfileApprovalLoading}
                  onClick={() => handleRejectRepProfileUpdate(repProfileApprovalModal.id)}
                  className="px-4 py-2.5 rounded-xl bg-rose-50 hover:bg-rose-100 text-rose-700 font-bold text-xs cursor-pointer transition border border-rose-200 flex items-center gap-1.5"
                >
                ...
                <button
                  type="button"
                  disabled={repProfileApprovalLoading}
                  onClick={() => handleApproveRepProfileUpdate(repProfileApprovalModal.id)}
                  className="px-6 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold text-xs shadow-lg shadow-emerald-600/20 cursor-pointer transition flex items-center gap-1.5"
                >
                  {repProfileApprovalLoading ? (
                    <>
                      <Loader2 className="w-4 h-4 animate-spin" />
                    ...
                    <>
                      <CheckCircle2 className="w-4 h-4" />
                      <span>অনুমোদন ও চূড়ান্ত আপডেট (Approve)</span>
                    </>
                  )}
                </button>
              </div>
          ...
          <div className="bg-white rounded-3xl p-6 max-w-md w-full shadow-2xl border border-slate-200 space-y-5">
            ...
            <div className="text-center space-y-2">
              <h3 className="text-lg font-black text-slate-900">
                সদস্যের তথ্য মুছে ফেলতে চান?
              </h3>
              <p className="text-xs text-slate-600 leading-relaxed">
                আপনি কি নিশ্চিত যে <strong>{memberToDelete.memberName}</strong> (কার্ড আইডি: <span className="font-mono font-bold text-rose-600">{memberToDelete.cardId}</span>)-এর প্রোফাইল ও সকল তথ্য সম্পূর্ণভাবে মুছে ফেলতে চান?
              </p>
              <div className="p-3 bg-rose-50 border border-rose-200 rounded-xl text-[11px] text-rose-700 font-medium">
                ⚠️ এই অ্যাকশনটি অপরিবর্তনযোগ্য। ডিলিট করার পর এই সদস্যের কোনো রেকর্ড বা কার্ড ডেটা পুনরুদ্ধার করা সম্ভব নয়।
              </div>
            </div>
            ...
            <div className="flex items-center justify-end gap-3 pt-2">
              <button
                ...
                className="px-4 py-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs cursor-pointer transition flex-1"
              >
                বাতিল করুন
              </button>
              <button
                ...
                className="px-5 py-2.5 rounded-xl bg-rose-600 hover:bg-rose-700 text-white font-black text-xs shadow-lg cursor-pointer transition flex-1 flex items-center justify-center gap-1.5"
              >
                {isDeletingMember ? (
                  <>
                    <Loader2 className="w-4 h-4 animate-spin" />
      ...
      {/* DIRECT ADD REPRESENTATIVE MODAL */}
      {showAddRepModal && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-3xl p-6 max-w-2xl w-full space-y-6 shadow-2xl animate-fadeIn my-8 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b pb-3 sticky top-0 bg-white z-10">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="font-black text-slate-900 text-base">নতুন ফিল্ড প্রতিনিধি যোগ করুন (Direct Add)</h3>
                  <p className="text-xs text-slate-500">প্রতিনিধি পোর্টালে সরাসরি লগইন ও টার্গেট অ্যাসাইন করার জন্য তথ্য পূরণ করুন</p>
                </div>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={handleDirectAddRepresentative} className="space-y-5 text-xs">
              {/* Personal Info */}
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 space-y-3">
                <h4 className="font-extrabold text-slate-800 text-xs border-b border-slate-200 pb-1.5 flex items-center gap-1.5">
                  <UserIcon className="w-4 h-4 text-sky-600" />
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    <label className="block font-bold text-slate-700 mb-1">প্রতিনিধির পুরো নাম *</label>
                    <input
                      type="text"
                      required
                  ...
                  <div>
                    <label className="block font-bold text-slate-700 mb-1">মোবাইল নম্বর (লগইন ইউজারনেম) *</label>
                    <input
                      type="text"
                      required
                      placeholder="017XXXXXXXX"
                      value={newRepForm.mobile}
                      onChange={e => setNewRepForm({ ...newRepForm, mobile: e.target.value })}
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      placeholder="19951234567890"
                      value={newRepForm.nidNumber}
                      onChange={e => setNewRepForm({ ...newRepForm, nidNumber: e.target.value })}
                      className="w-full p-2.5 rounded-xl border bg-white font-mono"
                    />
                  </div>
                </div>
              ...
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 space-y-3">
                <h4 className="font-extrabold text-slate-800 text-xs border-b border-slate-200 pb-1.5 flex items-center gap-1.5">
                  <ImageIcon className="w-4 h-4 text-emerald-600" />
                  প্রতিনিধির ছবি (Photo Upload / URL)
                </h4>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-12 gap-4 items-center">
                  <div className="sm:col-span-3 flex flex-col items-center justify-center p-2 bg-white rounded-xl border border-slate-200 text-center">
                    {newRepForm.photoUrl ? (
                      <img
                        src={newRepForm.photoUrl}
                        alt={newRepForm.name || 'Rep'}
                        className="w-16 h-16 rounded-full object-cover border-2 border-emerald-500 shadow-md"
                      />
                    ) : (
                      <div className="w-16 h-16 rounded-full bg-slate-100 border-2 border-dashed text-slate-400 flex items-center justify-center font-bold text-[10px]">
                        ছবি নাই
                      </div>
                    )}
                    <span className="text-[10px] text-slate-500 font-bold mt-1">প্রিভিউ</span>
                  </div>
                  ...
                  <div className="sm:col-span-9 space-y-2">
                    <div>
                      ...
                      <label className="px-3 py-1.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs flex items-center gap-1.5 cursor-pointer shadow-sm transition w-fit">
                        <Upload className="w-3.5 h-3.5" />
                        ...
                        <input
                          ...
                          accept="image/*"
                          className="hidden"
                          onChange={handleNewRepPhotoUpload}
                        />
                      </label>
                    </div>
                    <div>
                      <label className="block text-[11px] font-bold text-slate-700 mb-1">
                        অথবা ছবির অনলাইন লিংক (Image URL):
                      </label>
                      <input
                        type="text"
                        placeholder="https://..."
                        value={newRepForm.photoUrl}
                        onChange={e => setNewRepForm({ ...newRepForm, photoUrl: e.target.value })}
                        className="w-full p-2 rounded-xl border bg-white font-mono text-[11px]"
                      />
                    </div>
                  </div>
                </div>
              ...
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 space-y-3">
                <h4 className="font-extrabold text-slate-800 text-xs border-b border-slate-200 pb-1.5 flex items-center gap-1.5">
                  ...
                  কাজের দায়িত্বপ্রাপ্ত এলাকা ও টার্গেট
                </h4>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    <label className="block font-bold text-slate-700 mb-1">দায়িত্বপ্রাপ্ত জেলা *</label>
                    <input
                      type="text"
                      required
                      value={newRepForm.district}
                      onChange={e => setNewRepForm({ ...newRepForm, district: e.target.value })}
                      className="w-full p-2.5 rounded-xl border bg-white font-medium"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      type="text"
                      required
                      value={newRepForm.upazila}
                      onChange={e => setNewRepForm({ ...newRepForm, upazila: e.target.value })}
                      className="w-full p-2.5 rounded-xl border bg-white font-medium"
                    />
                  </div>
                  <div className="sm:col-span-2">
                    <label className="block font-bold text-slate-700 mb-1">নির্ধারিত কাজের নির্দিষ্ট এলাকা / ইউনিয়ন</label>
                    <input
                      type="text"
                      placeholder="যেমন: গোপালগঞ্জ পৌরসভা ও আশপাশের ইউনিয়ন"
                      value={newRepForm.assignedArea}
                      onChange={e => setNewRepForm({ ...newRepForm, assignedArea: e.target.value })}
                      className="w-full p-2.5 rounded-xl border bg-white font-medium"
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-3 pt-1">
                  <div>
                    <label className="block font-bold text-slate-700 mb-1">দৈনিক টার্গেট (কার্ড)</label>
                    <input
                      type="number"
                      min={0}
                      value={newRepForm.dailyTarget}
                      onChange={e => setNewRepForm({ ...newRepForm, dailyTarget: Number(e.target.value) })}
                      className="w-full p-2 rounded-xl border bg-white font-bold text-emerald-700"
                    />
                  </div>
                  <div>
                    <label className="block font-bold text-slate-700 mb-1">সাপ্তাহিক টার্গেট</label>
                    <input
                      type="number"
                      min={0}
                      value={newRepForm.weeklyTarget}
                      onChange={e => setNewRepForm({ ...newRepForm, weeklyTarget: Number(e.target.value) })}
                      className="w-full p-2 rounded-xl border bg-white font-bold text-sky-700"
                    />
                  </div>
                  <div>
                    <label className="block font-bold text-slate-700 mb-1">মাসিক টার্গেট</label>
                    <input
                      type="number"
                      min={0}
                      value={newRepForm.monthlyTarget}
                      onChange={e => setNewRepForm({ ...newRepForm, monthlyTarget: Number(e.target.value) })}
                      className="w-full p-2 rounded-xl border bg-white font-bold text-purple-700"
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center justify-end gap-2 pt-3 border-t">
                ...
                <button
                  ...
                  className="px-6 py-2.5 rounded-xl bg-sky-600 hover:bg-sky-700 text-white font-black text-xs shadow-lg cursor-pointer transition flex items-center gap-1.5"
                >
                  {isAddingRep ? (
                    <>
                      <Loader2 className="w-4 h-4 animate-spin" />
                      <span>প্রতিনিধি যুক্ত হচ্ছে...</span>
                    </>
                  ) : (
                    <>
                      <UserCheck className="w-4 h-4" />
                      <span>প্রতিনিধি যুক্ত করুন</span>
                    </>
                  )}
                </button>
              </div>
      ...
      {newRepSuccessAlert && (
        <div className="fixed inset-0 z-50 bg-slate-900/70 backdrop-blur-sm flex items-center justify-center p-4 animate-fadeIn">
          <div className="bg-white rounded-3xl p-6 max-w-md w-full shadow-2xl border border-slate-200 space-y-4 text-center">
            ...
            <div className="space-y-1.5">
              <h3 className="text-lg font-black text-slate-900">
                ফিল্ড প্রতিনিধি সফলভাবে যুক্ত হয়েছে!
              </h3>
              <p className="text-xs text-slate-600">
                <strong>{newRepSuccessAlert.name}</strong> প্রতিনিধি পোর্টালে সরাসরি লগইন করতে পারবেন।
              </p>
            </div>

            <div className="p-3 bg-slate-50 border border-slate-200 rounded-2xl text-left space-y-1 text-xs">
              <p>আইডি: <strong className="font-mono text-sky-700">{newRepSuccessAlert.id}</strong></p>
              <p>লগইন মোবাইল: <strong className="font-mono text-slate-900">{newRepSuccessAlert.mobile}</strong></p>
              <p>ডিফল্ট পাসওয়ার্ড: <strong className="font-mono text-emerald-700">123456</strong> (বা মোবাইল নম্বর)</p>
            </div>

            <button
              type="button"
              onClick={() => setNewRepSuccessAlert(null)}
              className="w-full py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs cursor-pointer shadow transition"
            >
              ঠিক আছে
            </button>
          </div>
      ...
      {paperCheckModal && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-3xl p-6 max-w-3xl w-full space-y-6 shadow-2xl animate-fadeIn my-8">
            ...
            <div className="p-4 bg-slate-900 text-white rounded-2xl border border-slate-700 space-y-3 text-xs">
              <div className="flex items-center justify-between border-b border-slate-700 pb-2">
                <div className="flex items-center gap-2">
                  <CreditCard className="w-4 h-4 text-amber-400" />
                  <span className="font-extrabold text-sm text-white">আবেদন ফি ও পেমেন্ট ভেরিফিকেশন (Admin Payment Verification)</span>
                </div>
                <span className={`px-2.5 py-0.5 rounded-full font-black text-[11px] ${
                  paperCheckModal.paymentStatus === 'PAID'
                    ? 'bg-emerald-500/20 border border-emerald-400 text-emerald-300'
                    : paperCheckModal.paymentStatus === 'PENDING'
                    ? 'bg-amber-500/20 border border-amber-400 text-amber-300'
                    : 'bg-rose-500/20 border border-rose-400 text-rose-300'
                }`}>
                  {paperCheckModal.paymentStatus === 'PAID' 
                    ? '✓ PAID (ভেরিফাইড ও পেইড)' 
                    : paperCheckModal.paymentStatus === 'PENDING' 
                      ? '⏳ PENDING (ভেরিফিকেশন অপেক্ষমাণ)' 
                      : '✕ UNPAID / REJECTED'}
                </span>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-[11px]">
                <div className="bg-slate-800/80 p-2.5 rounded-xl">
                  <span className="text-slate-400 block text-[10px]">ফি এর পরিমাণ:</span>
                  <strong className="text-amber-300 text-sm font-black">৳ {paperCheckModal.paymentAmount || 500} BDT</strong>
                </div>
                <div className="bg-slate-800/80 p-2.5 rounded-xl">
                  <span className="text-slate-400 block text-[10px]">পেমেন্ট গেটওয়ে:</span>
                  <strong className="text-white font-bold">{paperCheckModal.paymentMethod || 'N/A'}</strong>
                </div>
                <div className="bg-slate-800/80 p-2.5 rounded-xl">
                  <span className="text-slate-400 block text-[10px]">প্রেরক অ্যাকাউন্ট / বিবরণ:</span>
                  <strong className="text-sky-300 font-bold">{paperCheckModal.paymentAccountNo || 'N/A'}</strong>
                </div>
                <div className="bg-slate-800/80 p-2.5 rounded-xl">
                  <span className="text-slate-400 block text-[10px]">ট্রানজেকশন ID (TrxID):</span>
                  <strong className="text-emerald-300 font-mono font-extrabold">{paperCheckModal.paymentTxnId || 'N/A'}</strong>
                </div>
              </div>

              <div className="flex flex-wrap items-center justify-between gap-2 pt-2 border-t border-slate-800">
                <p className="text-[11px] text-slate-300">
                  * বিকাশ/নগদ/ব্যাংকের ট্রানজেকশন আইডি (TrxID) মিলিয়ে পেমেন্ট ভেরিফাই করুন।
                </p>
                <div className="flex items-center gap-2">
                  {paperCheckModal.paymentStatus !== 'PAID' && (
                    <button
                      type="button"
                      onClick={() => handleVerifyRepPayment(paperCheckModal.id, 'PAID')}
                      className="px-3.5 py-1.5 bg-emerald-500 hover:bg-emerald-600 text-slate-950 font-black rounded-xl text-xs flex items-center gap-1 shadow cursor-pointer transition"
                    >
                      <CheckCircle2 className="w-3.5 h-3.5" />
                      পেমেন্ট ভেরিফাই ও অনুমোদন (Approve Payment)
                    </button>
                  )}
                  {paperCheckModal.paymentStatus !== 'REJECTED' && (
                    <button
                      type="button"
                      onClick={() => handleVerifyRepPayment(paperCheckModal.id, 'REJECTED')}
                      className="px-3.5 py-1.5 bg-rose-500/20 hover:bg-rose-500/30 text-rose-300 border border-rose-500/40 font-bold rounded-xl text-xs flex items-center gap-1 cursor-pointer transition"
                    >
                      <X className="w-3.5 h-3.5" />
                      পেমেন্ট বাতিল / ভুয়া TrxID
                    </button>
                  )}
                </div>
              </div>
      ...
      <div id="bulk-cards-pdf-export-container" className="hidden space-y-8 bg-slate-200 p-4 font-sans">
        {(() => {
          ...
          return pages.map((pageCards, pageIdx) => (
            <div
              key={pageIdx}
              className="pdf-page-block bg-white flex flex-col justify-between mx-auto box-border overflow-hidden relative shadow-md"
              style={{
                width: '800px',
                height: '1131px',
                minWidth: '800px',
            ...
            >
              <div className="border-b pb-2 text-center flex-shrink-0" style={{ borderBottom: '1px solid #e2e8f0', paddingBottom: '8px' }}>
                <div className="flex items-center justify-between px-2 mb-1" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest" style={{ fontSize: '10px', color: '#94a3b8' }}>
                    DMB OFFICIAL CARD SHEET
                  </span>
                  <span className="text-[10px] font-bold text-emerald-700 bg-emerald-50 px-2.5 py-0.5 rounded border border-emerald-200" style={{ fontSize: '10px', color: '#047857', backgroundColor: '#ecfdf5', borderColor: '#a7f3d0' }}>
                    Page {pageIdx + 1} of {pages.length}
                  </span>
                </div>
                <h2 className="text-lg font-extrabold text-slate-900 tracking-tight leading-none" style={{ fontSize: '18px', fontWeight: '800', color: '#0f172a', margin: 0 }}>
                  {cardDesignSettings.headerTitle || 'DIGITAL MEDI BRIDGE'}
                </h2>
                <p className="text-[11px] text-slate-600 mt-0.5" style={{ fontSize: '11px', color: '#475569', margin: '2px 0 0 0' }}>
                  {cardDesignSettings.headerSubtitle || 'Healthcare Service Platform & Medical Network'}
                </p>
                <p className="text-[10px] font-bold text-emerald-700 mt-0.5" style={{ fontSize: '10px', color: '#047857', fontWeight: '700', margin: '2px 0 0 0' }}>
              ...
              <div
                className="text-center pt-2 border-t text-[10px] text-slate-500 flex justify-between items-center px-2 flex-shrink-0"
                style={{ borderTop: '1px solid #e2e8f0', paddingTop: '8px', fontSize: '10px', color: '#64748b', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}
              >
                <span>{cardDesignSettings.footerText || 'DMB Healthcare Network, Bangladesh'}</span>
                <span className="font-semibold text-slate-700" style={{ color: '#334155', fontWeight: '600' }}>Helpline: {cardDesignSettings.helpline}</span>
              </div>
            </div>
          ));
        })()}
